...
- ⏭ **4.5** Per-mode behaviour — fully specced 2026-05-09 across timer, save, submission, and re-entry; see runner.html §8 + §9.1 + §13 + §15 (settled) and attempt-creation.html §6.1.3 (revised). Five dimensions:
  - **Timer (§8 settled)** — unified clock pill: stopwatch for untimed (count-up, neutral, no warnings) / countdown for timed (count-down, escalating tone). `mm:ss` under 1 hr, `h:mm:ss` once over. Hide toggle in all 8 tuples (per-attempt scope, eye-icon next to pill, locked re-hide once first warning fires). Warning tiers 30 / 15 / 5 / 1 min with duration-conditional firing (30 needs ≥60 min duration, 15 needs ≥30 min, 5 needs ≥10 min, 1 always). Tone escalates only — never reverts. Stopwatch resets to 0:00 on Resume with "Resumed" tag (sidesteps engagement-clock complexity, deferred to v2).
  - **Save-on-tap (§9.1 settled)** — `nclex_save_progress` RPC moves from 4.6 to 4.5; becomes universal across all 7 active tuples (CAT excepted). Every material change writes a DRAFT row + appends to `answer_changes_json`. ~500ms client debounce. Replaces the per-Q-submit-creates-row pattern in UL too (backport for consistency + analytics value). Auto-submit on expiry becomes a single SQL UPDATE flipping all DRAFT rows to AUTO_SUBMITTED — no client-flush race.
  - **Submission archetypes (§15)** — three archetypes, not 8 per-tuple decisions. **A**: Untimed Learning (already shipped) — per-Q submit + immediate rationale + free nav. **B**: Free-batched (UT, TFN both intents) — per-Q submit removed; footer is `Next ›` until last Q's `Finish quiz`; revisable until Finish; confirmation modal if any blanks at Finish. **C**: Sequential (TS both intents) — per-Q `Submit & continue` locks the row; Prev disabled; grid locks answered cells; no Skip button (must commit, matches NCLEX authenticity).
  - **EXAM re-entry (attempt-creation §6.1.3 revised)** — was "exams cannot be resumed"; now "timed EXAM resumable mid-timer; wall-clock continues during absence; lazy expiry detection on page mount → TIMED_OUT → review." Untimed EXAM also resumable. CAT keeps its exception (IRT validity). 60s `last_activity_at` heuristic distinguishes re-entry from soft refresh.
  - **Case-exit warning (queued from 4.3)** — Archetype B free-nav only (sequential locks Prev, so the case never gets exited backward). Centred dialog; per-attempt suppression checkbox; backdrop click maps to "Stay in case." Suppression resets each attempt.
