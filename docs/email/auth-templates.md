...
| Template | Why not |
|---|---|
| **Invite** | Going custom — **half done as of 2026-08-12.** An invite always arrives attached to something, so the email worth sending is the one that says *what you've been given*, not *you have an account*. ✅ **The tutor-add path is swapped**: `lib/enrolments/actions.ts` now calls `generateLink({ type: 'invite' })`, Supabase sends nothing, and `enrolment.tutor_added` / `waitlist.converted` carry the link. ⬜ **The pay-first path is not**: `lib/payments/activate.ts` still calls `inviteUserByEmail`, so a guest who pays before having an account still gets Supabase's default body **alongside** our receipt — two emails for one action, which is the duplication the swap exists to remove. That one is riskier (money has already moved) and waits its turn. Both read **from** Quademia since the 2026-08-06 SMTP switch, so the remaining one is unstyled rather than untrustworthy. |
