...
- **Cohort-level payment plans (per-cohort override) — DESIGNED
  2026-06-22, not built.** A cohort can carry its own plan set
  (clone-and-edit) via a nullable `cohort_id` on the strategies table +
  a **Programme-pricing ↔ Custom** toggle on a new cohort Payment-plans
  pane; checkout reads cohort-effective plans (custom else programme,
  still frozen onto the enrolment); discovery shows the programme
  default. One migration. See *Tutored enrolment → Designed 2026-06-22 —
  cohort-level payment plans*.
...
#### Designed 2026-06-22 — cohort-level payment plans (per-cohort override; not built)
