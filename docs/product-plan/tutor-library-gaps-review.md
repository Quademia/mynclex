...
|---|---|---|---|
| **1. Reorder inside a shelf-attached block** | A shelf can be dropped into a unit as a grouped block. If the tutor drags one of those notes up *inside the unit*, should the shelf itself reorder, or is it a unit-local move? Without a rule, you get silent drift — the shelf says one order, the unit shows another, and nobody knows which is "right." | **Local-only overrides.** Reorder inside a unit changes that attachment's `position` and sets a new `is_position_overridden = TRUE` flag on the row. Future shelf reorders skip those rows. Show a tiny "moved" indicator on overridden rows, plus a "Reset to shelf order" action on the grouped-block kebab. The shelf detail page is the only place that reorders the shelf itself. | 🟡 |
...
| **14. Shelf changes after a shelf is attached** | If a tutor adds a new note to a shelf that's already attached to Unit 3, where does the new row appear in Unit 3? | **Mirror the shelf's order at the moment of add.** New attachment row's `position` = the note's position on the shelf. Existing rows with `is_position_overridden = TRUE` (from #1) keep their override; the new row slots in around them. "Re-sort to shelf order" action on the grouped block lets the tutor flush all overrides if needed. | 🟡 |
...
- `is_position_overridden BOOLEAN` (point 1)
...
- Add overridden / skipped row indicators (points 1, 2)
