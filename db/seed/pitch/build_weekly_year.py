...
def build():
    ...
    # -- 9. the demo student -------------------------------------------
    a("-- " + "=" * 68)
    a("-- 9. Claudia, enrolled")
    a("-- " + "=" * 68)
    a("-- TUTOR_ADDED rather than SELF_PAID, and not for flavour: a student")
    a("-- CANNOT self-join this programme today. Both public surfaces gate")
    a("-- enrolment on headline_price_minor > 0 —")
    a("--   app/(public)/programmes/[id]/page.tsx        (canEnrol)")
    a("--   app/(public)/checkout/programme/[id]/page.tsx (enrollable, then")
    a("--                                                  redirects away)")
    a("-- so a free programme shows 'Message the tutor' and a hand-typed")
    a("-- checkout URL bounces. The tutor adding them from the roster")
    a("-- (lib/enrolments/actions.ts) is the only path that exists, so that")
    a("-- is the one the seed uses. If free self-serve is ever built, this")
    a("-- becomes SELF_PAID with enrolled_by_user_id NULL.")
    a("--")
    a("-- enrolled_at is NOW(): she joins in week 9 and inherits the back")
    a("-- catalogue, which is the whole argument for allow_late_join. No")
    a("-- attendance rows — she has not sat a session yet, and inventing")
    a("-- history she did not have is how the register stopped matching the")
    a("-- enrolments last time.")
    a("--")
    a("-- ⚠ 07-demo-student.sql OPENS by deleting every enrolment this user")
    a("-- has. Re-running 07 after this file drops the row; re-run this file")
    a("-- afterwards. Guarded on the user existing so 09 does not hard-")
    a("-- depend on 07 having run at all.")
    a("")
    a("INSERT INTO nclex_enrolments (")
    a("  enrolment_id, user_id, programme_id, cohort_id, status,")
    a("  enrolment_source, enrolled_by_user_id, enrolled_at,")
    a("  approved_at, approved_by_user_id, strategy_id, access_expires_at")
    a(")")
    a("SELECT")
    a("  '71000000-0000-4000-8000-000000000912'::uuid,")
    a(f"  u.id, {q(PROGRAMME_ID)}, {q(COHORT_ID)}, 'ENROLLED',")
    a(f"  'TUTOR_ADDED', {q(TUTOR_ID)}, NOW(),")
    a(f"  NOW(), {q(TUTOR_ID)},")
    a("  '88000000-0000-4000-8000-000000000004', NULL")
    a("FROM   nclex_users u")
    a("WHERE  u.id = '385acbf4-fa50-4924-98f4-e1c46635ee1d'")
    a("ON CONFLICT (enrolment_id) DO UPDATE SET")
    a("  cohort_id = EXCLUDED.cohort_id, status = 'ENROLLED',")
    a("  enrolment_source = EXCLUDED.enrolment_source,")
    a("  enrolled_by_user_id = EXCLUDED.enrolled_by_user_id,")
    a("  approved_at = EXCLUDED.approved_at,")
    a("  approved_by_user_id = EXCLUDED.approved_by_user_id,")
    a("  strategy_id = EXCLUDED.strategy_id,")
    a("  access_expires_at = NULL, terminal_at = NULL,")
    a("  paused_at = NULL, paused_reason = NULL, updated_at = NOW();")
    a("")
    a("COMMIT;")
    a("")
