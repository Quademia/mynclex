-- =========================================================
-- MyNclex — Payments Slice 5.6: bank entitlement gate
-- File: mynclex/db/migrations/20260605120000_slice_5_6_bank_entitlement_gate.sql
-- =========================================================
-- Bank practice now requires an ACTIVE bank subscription. This is the DB
-- layer of the gate (the hard backstop): a SQL helper defining "active
-- bank access" + the create-attempt RPC refusing to start a bank attempt
-- without it. The TS page guard (requireActiveBankSubscription) and the
-- picker UI sit on top, reading the same definition.
--
-- "Active bank access" = an ACTIVE subscription of a bank-family pack
-- (BANK_DURATION or TRIAL) that hasn't expired. We compare end_at to now()
-- directly rather than trusting the status column alone, so the check is
-- correct even before the (deferred) expiry-sweep cron flips ACTIVE→EXPIRED.
-- A NULL end_at = lifetime (e.g. unactivated readiness / future grants).

CREATE OR REPLACE FUNCTION nclex_has_active_bank_access(p_user UUID)
RETURNS BOOLEAN
...
-- Internal helper — called by SECURITY DEFINER functions (which run as the
-- owner, so they keep EXECUTE). Not for direct client calls.
REVOKE EXECUTE ON FUNCTION nclex_has_active_bank_access(UUID) FROM anon, authenticated;


-- nclex_create_attempt re-emitted with the entitlement gate added right
-- after the auth/source checks. Body otherwise unchanged from slice 2.2a.
-- All attempts from this RPC are CUSTOM_BUILT (bank), so the gate applies
-- unconditionally; SUPER_ADMIN bypasses for tooling/testing.
