-- 20260714120000_trend_retire_flat_grid.sql
--
-- Trend rich multi-chart — Slice 4: retire the legacy flat grid.
--
-- Slices 1–3 built the rich multi-chart trend stimulus (nclex_trend_tabs)
-- alongside the old flat grid, keeping the flat grid as a fallback. The
-- flat grid is now retired: every trend is tab-based. This migration
--   (1) re-points nclex_create_attempt to stop freezing the flat-grid
--       columns into the attempt snapshot (tabs only, mirror of the case
--       freeze already in 20260713120000);
--   (2) drops the now-dead snapshot columns; and
--   (3) drops the dead flat-grid columns off both dataset tables.
--
-- Additive-playbook cleanup: the legacy flat-grid datasets themselves were
-- deleted as test data before this ran (none reached real users on dev or
-- prod), so there is nothing to convert — only dead columns + code to
-- remove. The app-layer flat-grid readers/writers are removed in the same
-- change set.

-- ─────────────────────────────────────────────────────────────
-- 1. nclex_create_attempt — drop the flat-grid freeze (tabs only).
--    Full body re-applied (CREATE OR REPLACE) with the trend-snapshot
--    INSERT trimmed to title/scenario/kind + tabs_snapshot_json.
-- ─────────────────────────────────────────────────────────────
  ...
  LOOP
    ...
    IF r_unit.unit_type = 'QUESTION' THEN
      INSERT INTO nclex_attempt_trend_snapshots (
        ...
        kind_snapshot, tabs_snapshot_json
      )
...
-- ─────────────────────────────────────────────────────────────
-- 2. Drop the now-dead flat-grid snapshot columns.
-- ─────────────────────────────────────────────────────────────
ALTER TABLE nclex_attempt_trend_snapshots
  DROP COLUMN row_label_snapshot,
  DROP COLUMN timepoints_snapshot_json,
  DROP COLUMN rows_snapshot_json;

-- ─────────────────────────────────────────────────────────────
-- 3. Drop the dead flat-grid columns off both dataset tables.
--    The stimulus lives entirely in nclex_trend_tabs now.
-- ─────────────────────────────────────────────────────────────
