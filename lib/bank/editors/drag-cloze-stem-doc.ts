// mynclex/lib/bank/editors/drag-cloze-stem-doc.ts
//
// DRAG_CLOZE rich-stem helpers — Option B, decoupled markers.
//
// The DRAG_CLOZE stem is a rich document; the [N] slot markers (single
// brackets — distinct from Highlight's [[double]] and Cloze's {N}) live as
// PLAIN TEXT inside its prose. Like Cloze's {N}, a drag-cloze [N] is a positional
// INDEX (it maps to slot sN), not displayed content — so there is no renumber
// step (the parser preserves markers byte-identical on save; gaps like
// [1] [3] are intentionally kept). These pure helpers are the "boundary" that
// keeps the decoupling honest:
//
//   - dragClozeStemScanText — flatten the doc to a scan string (the editor's live
//     slot reconciliation + the parser's marker scan read from this).
//   - normalizeDragClozeStem — the auto-tidy: strip any marks clinging to an [N]
...
// (lib/bank/parsers/drag-cloze.ts) + the editor.
const MARKER_G = /\[(\d+)\]/g;
 ...
 * before scanning.
 */
export function dragClozeStemScanText(doc: RichDoc): string {
 ...
 */
export function normalizeDragClozeStem(doc: RichDoc): RichDoc {
