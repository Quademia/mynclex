...
## Session — 2026-06-01 (later) — Study Home (11.14c) + Library Note as activity (11.11a/b, Option C) + cohort-checklist snapshot→live three-state redesign

> Logged in `2026-05.md` (not a new June file), continuing the prior
> call to keep the Tutor Library arc in one place. **All work on session
> branch `claude/elated-wiles-d0e067` — NOT merged to `main`.** Migrations
> `20260625120000` + `20260625130000` applied to **dev** only.

A long, discussion-heavy session. Three distinct pieces, plus a big
architecture reversal Sam drove.

### 11.14c — student library "Study Home"
Investigated whether the student library needed an overview like the
tutor side (it didn't have one — it dumped into a flat All-notes list).
Confirmed all feeders already exist (`nclex_library_note_state`
done/bookmark/visit + `nclex_library_embed_answers`), so **no migration**.
Built from a **Claude Design handoff delivered as an `api.anthropic.com/
v1/design/h/...` link** — it's a **gzipped tar** of the whole
`qacademy-tutor-library` CD project (WebFetch returns binary; `tar -xf`
it). Picked **Variant A (hero-led)** + generated-initials thumbnail.
New `lib/library/student/{home-queries,study-home,study-home-icons}`;
new scopes `home`(default)/`recent`/`bookmarked` in `scope.ts`; Recent +
Bookmarked sidebar views now real (2 of 3 placeholders retired);
visit-on-open stamp added to the read view. Commit `9f020c4`.

### 11.11a + 11.11b — Library Note as a curriculum activity (Option C)
Preceded by a **long alignment discussion** (Sam pushed hard — good).
Key reconstruction: the `nclex_tutor_library_note_attachments` table
(built in 11.1) was designed primarily for **shelves** (atomic one-row-
many-notes, can't be a `programme_activities` row); notes were folded in
for uniformity. The doc had a stale "write-through to `markActivityDone`"
line that contradicted the built attachments-only schema. **Decision
(Option C):** a Library Note attached to a unit is a **first-class
`nclex_programme_activities` row** (new `LIBRARY_NOTE` type) **AND** keeps
its attachment row, joined by a new `activity_id` column — so it inherits
ordering/publish/windows/picker chrome, while the attachment keeps the
note pointer + delete-protection FK + the existing "used in N" count.
Rejected A (attachments-only — re-grow all activity machinery) and B
(drop the table, payload pointer — would force re-engineering used-in +
delete-guard). **Completion stays DERIVED** from
`nclex_library_note_state` (11.11b fold-in in `student-queries.ts`), never
a progress-engine row (handles one-note-attached-to-many-units cleanly).
Migration `20260625120000` (LIBRARY_NOTE type + `activity_id` link).
New `lib/curriculum/{library-note-actions,library-note-attach-modal}`.
Live/Draft toggle on attach create AND edit (Sam caught I'd omitted it).
**11.11c (tutor embed-analytics dashboard) still ⬜.** Commit `22380b7`.

### Cohort checklist: snapshot → LIVE three-state model (Sam-driven)
Sam's testing surfaced that the 2 new library notes **and an older MOCK**
didn't appear in his cohort's checklist or to students. Root-caused: the
checklist was **seeded once at cohort creation** (trigger
`nclex_cohorts_seed_checklist`), with **no path** for activities added
later — documented-intentional (snapshot) but the intended "add from
template" affordance was deferred/never built. First built that affordance
(commit `92f3f32`), but Sam then proposed reversing the whole snapshot
model — **correctly**. Final model (after a thorough back-and-forth):
- Checklist = **live programme template**; a `nclex_cohort_checklist_items`
  row is only an **override**, created on first explicit decision.
- Three states per activity: **Unconfigured** (no row, hidden from
  students, faint default date) / **Included** / **Excluded**, via an
  explicit **`✓ Include / ✗ Exclude` segment** (neither = unconfigured).
- Inclusion/date edits are `(cohort_id, activity_id)` **upserts**
  (`applyChecklistChange`); **"N unconfigured · Include all"** prompt.
- Seed trigger **dropped** (`20260625130000`); existing cohorts keep
  their rows; new cohorts start all-unconfigured.
- **Student side unchanged** — already inner-joins included rows, so
  unconfigured/excluded are naturally hidden.
- The explicit ✗ was a follow-up after Sam noted that without it,
  reaching "excluded" needed a tick-then-untick dance and the
  unconfigured nag couldn't be cleanly resolved.
Commits `badd4b4` (live three-state) + `ad289dc` (Include/Exclude segment).
Doc: `curriculum-authoring-ux.md` §11 build-update note added.

### Status / next
Nothing merged. ⏭ after Sam tests + approves: merge branch → `main`.
Remaining library: **11.11c** (embed-analytics) → **11.12** (Shelf
activity) → **11.17** (polish). Stale-design note: §11's "Remove from
cohort / Add from template / cohort-only" affordances are not built
(cohort-only still future via the `source` column).

---

## Session — 2026-06-01 (Student-reading side of the library: 11.14a/b front door + 11.13a read view + 11.13b embed player — all MERGED to main)

> Logged in `2026-05.md` (not a new June file) by Sam's call — keeps the
> whole Tutor Library arc tracked in one place.

A long session that built the **entire student-reading half** of the
library, plus a deep design conversation before the embed player. Build
order followed the re-sequence (directly-library first): 11.14 → 11.13.
All merged to `main` via fast-forward.

### 11.14 a/b — student library (the front door)
Read-only mirror of the tutor lensed home, scoped to the programme's
tutor, in a new `lib/library/student/` folder (Sam approved; the
existing tutor files can move to `lib/library/tutor/` later as its own
refactor). **No migration** — the student-read DB floor already existed
(`nclex_student_can_see_note` + student-select RLS on
notes/folders/shelves/memberships + the `nclex_library_note_state`
table), so RLS does the visibility filtering and the queries just read.
- **14a** — `getStudentLibrarySnapshot(programmeId)` (resolve programme →
  tutor; fetch visible folders/shelves/notes); `StudentLibraryShell`
  (five-lens sidebar, read-only adaptations, hide-empty containers,
  client-side scope filtering, in-pane search); `StudentNoteRow`
  (links to the read view); route `/student/programme/[id]/library`;
  nav entry (`file-text` icon). Views = All notes (live) + Recent / By
  unit / Bookmarked (disabled placeholders pending feeders).
- **14b** — the **cohort (tutor-led) sibling** route
  `/student/cohort/[id]/library`; shell generalised `programmeId →
  basePath`; shared server-safe `scope.ts` parser; **collapse-to-rail**
  via `useSyncExternalStore` (SSR-safe, multi-tab, sidesteps the
  `set-state-in-effect` lint the tutor home carries). `lastPlay` →
  `sittings` later. `getStudentLibrarySnapshotForCohort` resolves cohort
  → programme → tutor.

### 11.13a — note read view
Full-page route `…/library/note/[note_id]` (programme + cohort
siblings). Reached the conclusion (confirmed with Sam) that the renderer
is a **custom JSON walker, NOT Tiptap** (lighter for students, no
authoring affordances, embed becomes a real player). Pieces:
`read-inline.tsx` (every formatting mark: bold/italic/underline/strike/
code/link/highlight-colour/text-colour/sub/sup), `read-blocks.tsx`
(dispatcher + heading-anchor/paragraph/lists/quote/code/rule inline;
callout/drug-card/lab-values/table reuse the editor's standalone
`.lib-*` classes; `.lib-read-prose` mirrors the ProseMirror prose CSS
since that's editor-scoped), `read-media-blocks.tsx` (image/pdf via
**enrolment-gated** signed-URL actions — the tutor action gates on
ownership which a student lacks; video embed/link-card). `read-note-view`
shell: Contents rail (scroll-spy on `.product-content` + "section N of
M" + resume via `last_heading_id`), bookmark + Mark-as-done
(`nclex_library_note_state`, write-through to the progress engine
stubbed until 11.11), reading-time estimate. **Follow-ons same session:**
breadcrumb (Library / folder) replacing the flat back-link + **all meta
chips clickable** back to `?shelf=`/`?pillar=`/`?tag=`; **named shelf
chips** (dot + title) replacing bare colour-dots on **both** tutor
(`NoteLensRow`) and student rows. New tables: none.

### 11.13b — embedded-questions player + attempt history (the differentiator)
Preceded by a long design discussion that **changed the model
substantially** from the original plan — worth recording:
- **Append-only history, not freeze-and-lock.** The live note always
  shows the tutor's current questions, answerable; each submit appends a
  row. Re-practising is allowed. This dissolved every "tutor edited the
  questions later" edge case (live = current, history = frozen per
  attempt) instead of coding around them.
- **Table fuses two runner tables.** Sam's catch: base it on
  `nclex_attempt_answers` — which surfaced the snapshot table
  (`nclex_attempt_items`) the first cut had dropped. The result inlines
  the answer fields + the snapshot columns into one row + note_id/
  block_id. FK item_id → `nclex_tutor_questions` (not `nclex_bank_items`
  as the old draft said).
- **`play_id`** (Sam's catch) groups a sitting; one per run of a block.
- **No fabricated rows** for skipped questions (would poison the analytics
  the history exists for); **no block-until-finished** (breaks on tutor
  edits). A partial play is an honest partial sitting.
- **Student-facing history** lives on the block's intro card (same
  player, a `review` state), not a separate page.

Build (4 checkpoints): migrations `20260624120000` (the table + RLS +
block-id backfill) + `20260624130000` (`play_id` + index), applied to
dev. Embed block node gained a generated `id` (freshEmbed + self-heal).
`embed-player-actions.ts` — secure `loadEmbedBlock` (answerable content,
no key, + past sittings), `submitEmbedAnswer` (service-role read →
`scoreAttempt` → snapshot row → feedback), `loadEmbedPlayReview`
(student-own rows). `embed-player.tsx` — intro/Start card (lists past
sittings, "Start again" = new play) → fresh pass (reuses bank-runner
MCQ/TF/SATA/SELECT_N + RationaleBlock, type-dispatched) → end summary;
read-only `review` state replays a sitting from its snapshot, paged.
Leave-mid-set guard via an `EmbedPlayGuard` context (player reports
mid-play → `read-note-view` does beforeunload + intercepts its own
back/breadcrumb/chip links; global sidebar not interceptable in App
Router). Late fix: hide the review pager's Prev/Next at the ends.

**Files new:** `lib/library/student/{queries,scope,student-library-shell,
student-note-row,note-read-queries,note-read-actions,read-media-actions,
read-inline,read-blocks,read-media-blocks,read-note-view,embed-player,
embed-player-actions,embed-play-guard}.tsx/.ts` + routes under
`app/(app)/student/{programme,cohort}/[...]/library/...` + migrations
`20260618…`-style `20260624120000` / `20260624130000`. **Modified:**
`note-lens-row.tsx`, `embed-block.tsx`, `lib/nav/student.ts`,
`styles/library.css`, `docs/product-plan/tutor-library.md`.

**Next:** programme integration **11.11** (Library Note activity) →
**11.12** (Shelf activity) → **11.17** (polish). Prod migrations to ship
at next release: `20260624120000` + `20260624130000`.

---

## Session — 2026-05-31 (Library 11.16c custom views + scroll-cap pass + pane Edit/Delete → whole 11.16 arc MERGED to main)

Closed out **11.16** and merged the whole arc to `main` (FF
`bd68889..11f59ae`, pushed). 11.16a/b had landed on `main` earlier in
the day; this session built 11.16c (10 commits on
`claude/heuristic-engelbart-c60f36`).

### 11.16c-1 — filter builder
`?view=new` pane: a filter bar over **status** (Any / Published /
Drafts) + **pillars** + **tags**, with the note list filtering live in
the browser over the full lens-row set (`getAllLensRowsForTutor`). AND
across dimensions, OR within pillars/tags. Pure model in
`lib/library/view-filters.ts` (`matchesViewFilters` /
`applyViewFilters` / `normalizeFilters` / `describeFilters`). New
`tag-multi-picker.tsx` mirrors `PillarPicker`. Scope agreed with Sam:
status+pillars+tags only — folder/shelf are already one-click scopes;
saved free-text search deferred.

### 11.16c-2 — persistence
`nclex_tutor_library_views` (migration `20260623120300`; UUID PK,
`tutor_id → nclex_users` ON DELETE CASCADE, `name`, `filters_json`
JSONB, `position`; `(tutor_id, position)` index; self + admin RLS
mirroring the sibling library tables). The table pre-existed on dev
from an exploratory run with an `auth.users` FK + role-unscoped
policies — reconciled dev **and** the migration file to the
`nclex_users` + `TO authenticated` sibling pattern (`a34830d`).
`view-actions.ts` = create / update / rename / delete (plain
RLS-scoped CRUD; `filters_json` sanitised through `normalizeFilters`).
Builder gained name + Save; saved views render in the Views lens with
live match counts; `?view=<uuid>` read pane (`saved-view-pane.tsx`)
carries a read-only filter summary + Edit filters / Rename (inline) /
Delete (confirm); stale uuid → "View not found".

### 11.16c-3 — sidebar scroll-cap pass
Folders / Shelves / Views per-item lists scroll within a 260px
`.lens-scroll`; the "All …" anchors + 4 system views + "+ New view"
sit outside it, pinned. Row kebab menus (folder + shelf ⋮) would've
been clipped by the new `overflow` container — converted from
`position:absolute` to fixed positioning via the existing
`usePopoverPosition` hook (anchored to the kebab rect, flips above near
the viewport bottom).

### Follow-on (Sam's ask) — folder/shelf pane Edit/Delete
Saved-view panes carry management in the header, but folder + shelf
detail panes only exposed CRUD via the sidebar kebab. Added Edit
(reopens the existing create/edit modal) + Delete (red) to both panes.
Extracted `DeleteShelfConfirm` out of `shelf-rows.tsx` into its own
file so the kebab + pane share it (mirrors the earlier
`RemoveFromShelfConfirm` pull-out); both delete confirms gained an
optional `redirectTo` so a delete from the detail pane routes to
`?folder=all` / `?shelf=all` instead of refreshing a now-404 URL. The
sidebar kebab keeps its in-place refresh.

### Process notes
Committed `8fd504a` before verifying; two `shelf-detail.tsx` edits had
silently no-matched and a `react-hooks/refs` lint slip rode along —
caught via tsc + eslint, fixed in `e36af9e` / `52b0995`. tsc + eslint
clean at merge.

### Files
New: `view-filters.ts`, `tag-multi-picker.tsx`, `view-builder.tsx`,
`view-actions.ts`, `saved-view-pane.tsx`, `delete-shelf-confirm.tsx`,
`db/migrations/20260623120300_slice_11_16c_custom_views.sql`.
Modified: `queries.ts`, `page.tsx`, `home-shell.tsx`, `folder-rows.tsx`,
`shelf-rows.tsx`, `notes-list.tsx`, `shelf-detail.tsx`,
`delete-folder-confirm.tsx`, `styles/library.css`, + the three docs.

### Next
Per the re-sequenced build order: **11.13** (student read-mode
renderer) — where 11.10's publish + visibility first become visible to
a student.

---

## Session — 2026-05-31 (Library 11.16a search + 11.16b tags; on session branch, NOT merged)

Built the first two-thirds of **11.16** across several tested sub-slices
on the session branch. **Nothing merged to `main` this session** — Sam
tested each piece at localhost and approved; the whole 11.16 arc merges
together once 11.16c lands. 7 commits on `claude/festive-engelbart-572b8c`.

### 11.16a — content search (prefix + live)

- **Indexer bug found + fixed (the session's headline).** Sam recalled
  "some blocks weren't searchable." Investigation showed it was worse:
  `nclex_extract_body_text` (written in 11.1, before Tiptap) bailed on
  its first line because the stored body is now a `{type:'doc',
  content:[...]}` OBJECT, not the top-level array it expected — so
  **every rich-editor note had ZERO body text indexed** (only
  title/subtitle/description searchable). Even past that, per-block
  branches read wrong keys (headings, `bulletList`/`orderedList`,
  `table` rows/cells, `drug_card`/`lab_values` attrs).
  - Rewrote it as a **single recursive walker** (Tiptap keeps prose in
    `{type:'text'}` leaves under `content`; the two atoms read from
    `attrs`; image/pdf/video/embedded_questions skipped). Since
    `body_tsv` is a STORED generated column, the migration DROPs +
    RE-ADDs it (forces recompute = retroactive backfill) + rebuilds the
    GIN index. Verified: NSR 0→3488 chars; digoxin/acidosis/bradycardia
    (drug card / lab grid / list) all now match. Migration
    `20260623120000`.
- **Search RPC** `nclex_search_library_notes` (SECURITY INVOKER,
  RLS-scoped) + `searchNotesForTutor` (RPC → ranked ids → batched
  lens-row projection). `?q=` (+ `?qf=`) URL scope, top precedence.
  `<LibrarySearchBox>` + `<SearchResults>`; per-field scope chips
  (Title/Subtitle/Description/Body) via a ts_rank weight mask off the
  existing A/B/C/D weighting — no extra storage. Field encode/decode in
  `search-fields.ts`.
- **11.16a-2 (prefix + live), after Sam flagged "must type the full
  word".** Migration `20260623120100`: the RPC now builds a PREFIX
  tsquery (`token:*`). Stemming-vs-prefix trap ("brady"→stem "bradi"
  fails to prefix "bradycardia"; but dropping stemming breaks full
  words like furosemide→stem "furosemid") solved by matching on
  **english (stemmed) prefix OR simple (literal) prefix**, ranking by
  the higher. Client: debounced (250ms) `router.replace` once ≥2 chars;
  Enter = `router.push`. Sam: "feels good."

### Tag-input discoverability fix (not a numbered slice)

Sam couldn't find where to add a tag. The editor's tag box was a
borderless field whose "+ tag" hint showed **only when zero tags
existed**. Made the **"+ Add tag" placeholder always visible** + styled
the input as a dashed accent pill mirroring the "+ Add pillar" chip
beside it (widens + turns solid on focus). Tags are added per-note in
the editor's meta row — there's no central "create tag" step (which is
exactly why the manager matters).

### 11.16b — tags (clickable lens + manager)

- **11.16b-1 — clickable Tags lens.** The placeholder Tags section now
  lists distinct tags + counts (`aggregateTags`, sorted count-then-alpha
  from the cached lens-row fetch); clicking filters via a new `?tag=`
  scope (precedence search > shelf > view > tag > folder > overview).
  `<TagRows>` (scroll-capped at 280px, # prefix, active highlight) +
  `<TagView>` main pane.
- **11.16b-2 — Manage tags panel (all three tools).** ⋮ on the Tags
  header opens `<ManageTagsModal>`: **rename / delete / merge**, each a
  bulk action across the tutor's notes. Migration `20260623120200`:
  order-preserving `nclex_dedupe_tags` helper + 3 SECURITY INVOKER RPCs
  (RLS self_update scopes writes; `updated_at` untouched — housekeeping,
  not an edit). Actions return affected-note counts for an inline status
  line; `router.refresh()` updates the list. Rename-onto-existing
  behaves as a merge (dedupe). Verified the transform logic on
  controlled inputs.

### Deferred follow-ons captured this session

- **Sidebar scroll-cap pass (do WITH 11.16c).** Tags already caps its
  list (280px + internal scroll). Apply the same to **Folders, Shelves,
  and Views** — the latter because **custom views (11.16c) make Views
  unbounded**, and custom-view rows get their own edit/delete kebab.
  Wrinkle: an `overflow:auto` container **clips** the row kebab popovers
  (Tags rows had no popover, so it dodged this). Fix = float the kebab
  menu out of the scroll box via fixed/anchored positioning (the
  `use-popover-position` helper already exists). Pin the "All folders" /
  "All shelves" anchor above the scroll. Pillars stays fixed (8 hard-
  coded). Bundle all of this into the 11.16c session so the clipping is
  solved once.
- **Tag autocomplete (reduce drift at source).** Suggest existing tags
  as the tutor types in the editor's tag box, so they reuse `furosemide`
  instead of fat-fingering a near-duplicate. Small, separate; pairs with
  the manager which fixes drift after the fact.

### Next session

**11.16c — custom views** (save a filter combination as a reusable
sidebar view; per-view edit/rename/delete) **+ the sidebar scroll-cap
pass** above, then **merge the whole 11.16 arc to `main`** (with Sam's
approval). 11.16 migrations to land on prod at release:
`20260623120000` / `120100` / `120200`.

---

## Session — 2026-05-30 (Library 11.10 Publish flow → MERGED; 11.15 Embedded-questions authoring half built; build-order re-sequenced)

Two arcs. **11.10 (Publish flow + visibility + alt-text preflight)**
was built, tested by Sam, and **merged to `main`** (`1bf74f3`). Then a
long design conversation re-sequenced the library build order and
specced **11.15 (Embedded questions)**, whose **tutor authoring half**
was built across five sub-slices (a–e) — **on the session branch, not
yet merged** as of session end (last commit `70b3f5c`).

### 11.10 — Publish flow (MERGED to main)

The DB floor was already in place from 11.1 (`is_published`,
`visibility_mode`, the `_note_visibility` junction, same-tutor +
deferred scoped-≥1 triggers), so this was almost all app-layer.

- **Atomic publish RPC** `nclex_set_library_note_publish` (migration
  `20260620120000`) — flips the flags + rewrites the junction in one
  transaction (the deferred constraint only validates at commit;
  a multi-REST approach would trip it or briefly over-expose a scoped
  note). SECURITY INVOKER + an explicit `auth.uid()` ownership guard.
  Verified end-to-end on dev (publish wide → scope 2 → re-scope 1 →
  cross-tutor reject → back to wide).
- **Actions**: `publishNoteAction` (doubles as re-scope; alt-text
  backstop + same-tutor programme check), `unpublishNoteAction`
  (junction kept so re-publish remembers scope).
- **Publish dialog** (Tutor-wide / Programme-scoped + programme
  checklist). **Alt-text preflight** (client) scans the live doc for
  `libImage` with empty alt → blocks Publish + scrolls to the first
  offender (`countMissingAltImages`). Editor wiring: live Publish /
  Visibility / Unpublish, real State + Visibility rail rows.

### Design: build-order re-sequence + cohort-visibility parked

- **Build order revised (Sam's call):** finish the library *as a
  library* first; **programme integration (11.11/11.12) builds LAST**.
  Numbers kept as stable IDs (the slash menu shows "11.15" to tutors),
  unbuilt entries listed in build order: 11.15 → 11.16 → 11.13 → 11.14
  → 11.11 → 11.12 → 11.17.
- **Cohort-level note visibility parked** — the junction is
  programme-keyed, so a note can't be scoped to one cohort within a
  programme. Deliberate + consistent with curriculum. Recorded as a
  deferred follow-on.

### 11.15 — Embedded questions, authoring half (a–e, NOT merged)

Built from a CD prototype (`embed-authoring/` in Sam's zip). Key
design decisions reached with Sam:

- **Reuse, not rebuild.** Pick from the tutor's own bank (the quiz
  picker pattern) AND author inline via the **existing bank editors**,
  stamped with a new `parent_note_id` (the trend-child pattern). But
  **unlike case/trend children, note-created questions stay
  builder-visible + default published** — they're standalone reusable
  bank questions, just labelled "Note-created". `parent_note_id` added
  to BOTH question tables (column symmetry).
- **v1 types = the 4 classic** (MCQ/SATA/TF/SELECT_N). NGN deferred —
  the runner *can* draw them, but their partial-credit grading doesn't
  fit the embed answer model.
- **Caps reframed (Sam):** instead of summing questions, **max 10 per
  block + max 5 blocks per note** (50 falls out). Enforced at the point
  of action (picker disables; insertion disables). 
- **The twist — caps are admin config.** Both limits are rows in
  `nclex_config`, editable on `/admin/config` (new `integer` config
  type), so Sam can re-dial them with no deploy. Threaded server→client.
- **Grandfather-safe** server backstop: checks absolute ceilings
  (30/10), never the live config — lowering a limit never blocks an
  existing over-limit note from saving.

Sub-slices: a (foundation) · b (block shell + Add menu) · c (picker +
reference cards) · d (create-inline) · e-1/2/3 (config + thread/enforce
+ backstop) + the grandfather fix. Typecheck clean throughout. Pending
Sam's final test + a `main` merge.

---

## Session — 2026-05-30 (Library Slices 11.7 + 11.8 + 11.9 — the NCLEX-domain "nursing-shaped" block trio: Callout, Drug card, Lab values)

Built all three NCLEX-domain blocks in one session, each iterated
through a CD-style read-back illustration before touching the editor.
The slash menu now has just one disabled row left (11.15 Embedded
questions). Commits `caecf15` (11.7) + `799018c` (11.8) + `b3a24da`
(11.9), fast-forwarded to `origin/main` at `b3a24da`.

### 11.7 — Callout block

A Tiptap **content node** (`callout`, `inline*` rich text) carrying a
`tone` attr — Note · Tip · Warning · Critical · Memory — that drives
the whole box colour plus the header icon + label. No custom title:
the label *is* the tone. Unlike the media blocks, the callout body is
real ProseMirror content, so the toolbar marks (bold/italic/link)
work natively inside it.

- **Header treatment iterated with Sam** (two sketched variants). We
  landed on a deep-fill **tab chip** straddling the top-left edge
  (icon + label, white on the deep tone), over a 4px left accent +
  soft body tint. The chip doubles as the tone switcher (click →
  5-tone dropdown → chip/accent/body re-colour together). Chosen over
  a full-width deep title-bar so stacked callouts stay calm in a long
  note and don't out-shout the H2/H3 headings.
- **No DB change** — the slice-11.1 `nclex_extract_body_text` helper
  already had a `callout` branch expecting `inline*` content, so
  callout text is searchable (naming the node plainly `callout`, not
  `libCallout`, is what lines it up).
- `defining: true` gives clean Enter-to-exit behaviour.
- New: `lib/library/callout-block.tsx`.

### 11.8 — Drug card block

A sealed **atom node** (`drug_card`) holding `{ name, drug_class,
fields: [{ label, value }] }` — all data in attrs, edited via a React
form in the NodeView (the Image-block pattern, bigger form). New
cards seed the 4 NCLEX-canonical fields; rename / ▲▼ reorder (drag
deferred) / add / remove.

- **CD design** from Sam's fresh handoff (the sodium-bicarbonate
  card): white spec-sheet, gradient header, 💊 + italic Rx flourish,
  Georgia-serif name, grey label-column / value grid — mapped to our
  teal + navy tokens, not the mock's indigo.
- **V1 plain-text values.** Discussed at length: allowing inline
  marks would turn each value into a nested editable region
  (Table-block-level complexity), so it's parked. Sam noted editing
  *would* feel more natural — captured as a v2 follow-on (below).
- Value boxes auto-grow; **label cells wrap** (auto-grow textarea,
  Enter suppressed, pasted newlines stripped) after Sam caught
  "Nursing considerations" clipping in the 170px label column.
- New: `lib/library/drug-card-block.tsx`.

### 11.9 — Lab values block

A sealed **atom node** (`lab_values`) holding `{ title, columns:
[{ label }], rows: [[…]] }` — an editable 2-D grid. New tables seed
the 4 canonical columns (Test · Normal · If high · If low) + two
blank rows; rename / add / remove columns and add / remove rows.

- **Inline grid controls:** column × (hover) with a centred
  **removal-warning dialog** — it's the destructive op (wipes the
  column's values across every row); ＋ add-column in the top-right
  header cell; row × (hover); dashed + Add row at the foot. The last
  column / last row lose their × (UI floor of ≥1 each).
- Auto-grow wrapping cells + column labels. CD design (gradient
  header + 🧪 + serif title, grey column headers, emphasised navy
  first column) on app tokens.
- New: `lib/library/lab-values-block.tsx`.

### Shared

`AutoGrowTextarea` (auto-sizing textarea, with a `singleLine` mode
that wraps but rejects Enter) was factored out of the drug card into
`lib/library/auto-grow-textarea.tsx` and reused by lab values.

### Posture (all three)

V1 plain text where applicable · soft validation only (the hard gate
— name/title required, ≥1 field/column/row, labels required — lands
with the publish preflight in 11.10) · no DB change.

### Deferred follow-ons captured

1. **Rich-text drug-card (and lab-values) field values.** Sam liked
   the idea twice; parked because it's a real rebuild (nested
   editable regions per cell). v2 candidate — revisit if tutors ask.
2. **Search-sync cleanup.** The form-style blocks (Callout aside)
   store their text under Tiptap's `attrs`, but the slice-11.1
   `nclex_extract_body_text` helper reads several block types at the
   JSON top level (and looks for pre-Tiptap names like `quote`,
   `image`, `drug_card`). So drug-card / lab-values / image / pdf /
   table text isn't indexed. Worth one slice that re-syncs the helper
   to the real Tiptap shape across all block types (needs a migration
   — the helper feeds a generated column).

### Process note

Each block was confirmed via a throwaway HTML read-back illustration
(opened in the browser) before the editor build — the callout's two
header variants, the drug card's read+edit views, and the lab table's
read+edit+warning. This kept the visual decisions ahead of the code
and matched well to Sam's no-code-background workflow.

---

## Session — 2026-05-30 (Library Slices 11.6b + 11.6c — PDF + Video blocks, then the Table block + colour-theme fix + note recovery + worktree cleanup)

Closed out the standard visual-block set for the tutor library
editor: **11.6b** added the PDF and Video blocks, **11.6c** added
the Table block. With these the 11.6 media group is complete —
Image · PDF · Video · Table. Both slices shipped to `main` this
session (commits `02cb7fb` + `1b362b6`, fast-forwarded to
`origin/main` at `1b362b6`).

### 11.6b — PDF block

A new private Storage bucket `nclex-library-pdfs` (migration
`20260619120000_slice_11_6b_library_pdfs.sql`) and a `LIBRARY_PDF`
purpose in the shared media config. The Tiptap atom node stores
only the `assetId`; the bytes live in Storage and the viewer mints
a 1-hour signed URL on demand (nothing public, no URL persisted) —
the same pattern as the image block. The NodeView
(`lib/library/pdf-block.tsx` + `pdf-actions.ts`) shows an upload
dropzone in the empty state and a filled link card once a PDF is
attached.

### 11.6b — Video block

`lib/library/video-block.tsx` + `video-embed.ts`. Videos are never
uploaded — the tutor pastes a URL, and the host is classified:

- **Safe inline embed** (YouTube / Vimeo) → rendered as an iframe.
- **Link card** (any other host) → a styled clickable card.
- **Rejected** → unsafe / unparseable URLs are refused.

### Universal link fallback

Both PDF and video degrade to a styled link card whenever an
inline view isn't possible, so a tutor never hits a dead end.

### Shared upload event

`lib/library/block-upload-event.ts` factors the autosave
"upload in flight" window-event (introduced for the image block in
11.6a) out into a shared helper, so PDF uploads gate autosave the
same way — uploading still isn't treated as inactivity.

### 11.6c — Table block

Built on `@tiptap/extension-table` v3 (`lib/library/table-block.ts`
+ `table-toolbar.tsx`). A floating contextual toolbar (Tiptap
BubbleMenu + Floating UI) anchors to the table while the cursor is
inside it and offers: add/remove row & column, merge/split cells,
header + sub-header row tagging, and six colour themes. The two
custom attributes — `colorTheme` on the table, `isSubheader` on a
row — ride Tiptap's generic `updateAttributes`, so no bespoke
commands are needed. Both are plain JSON scalars, so they survive
the `tiptapToBody` deep-clone at the Server Action boundary.

Decision: **no bonded title/subtitle.** An experiment to bond an
editable title + subtitle above the grid (a `libTableFigure`
wrapper) was built and then reverted — Sam's call: "the title and
subtitle will give us headache… if they want it to be a title or
subtitle they can merge" the top row instead. Keeps the table to
the strict prosemirror-tables structure.

### The colour-theme bug (the session's debugging headline)

Picking a colour swatch changed nothing visible — not even on a
header row. Root cause was **not** in our code: prosemirror-tables
ships a `TableView` node view that builds the `<table>` element by
hand (`document.createElement`) and copies only `node.attrs.style`
onto it. It ignores every custom attribute's `renderHTML`, so our
`data-color` attribute never reached the DOM and the theme CSS
(`table[data-color='blue']` …) had nothing to match. The node view
exists only for live column resizing, which we'd turned off
(`resizable: false`) — so it was contributing nothing but this bug.
Fix: override `addNodeView()` to return `null`, which falls back to
ProseMirror rendering the table from the extension's `renderHTML`
(which **does** emit `data-color`). Documented inline in
`table-block.ts`.

### Note recovery (data-loss scare, no data lost)

A template note Sam was using ("Normal Sinus Rhythm (NSR)") stopped
rendering — it appeared wiped. Cause: it had been saved during the
title/subtitle experiment, so its body contained `libTableFigure` /
`libTableTitle` / `libTableSubtitle` nodes that the reverted schema
no longer knows. ProseMirror refuses to render a document
containing unknown node types, so the whole note went blank — but
every byte was intact in the DB. Recovered in place: backed the
body up to `nclex_note_body_backup_tmp` first, then ran a jsonb
transform converting the single `libTableFigure` into three plain
top-level nodes — a level-3 **heading** (from the title text), a
**paragraph** (from the subtitle text), and the **table** — keeping
all of Sam's typed content. Confirmed rendering, then dropped the
backup table.

### Worktree + branch cleanup

End-of-session housekeeping: 52 worktrees had accumulated. Analysed
every old session branch against `origin/main` — all 50 were fully
merged (zero unmerged work anywhere). Removed all 50 stale
worktrees and deleted 51 merged branches (the `claude/*` set, the
leftover `claude/dev`, an orphaned `claude/kind-galileo`, and the
long-retired `work` branch). Only the `main` checkout + the active
session worktree remain; `origin` still carries just `main` + `prod`.

### Files

**New (8):** `db/migrations/20260619120000_slice_11_6b_library_pdfs.sql`,
...
**Modified:** `lib/library/note-body-editor.tsx`, `note-editor.tsx`,
`slash-menu.tsx`, `image-block.tsx`, `lib/media/types.ts`,
`lib/nav/types.ts`, `components/nav/shared/nav-icon.tsx`,
`styles/library.css`, `docs/product-plan/tutor-library.md`,
`package.json` + `package-lock.json` (`@tiptap/extension-table`).
...
## Session — 2026-05-29 (Library Slice 11.6a — Image block + image-persistence Server-Action serialization fix + upload-aware autosave)

Shipped the first of the editor's media blocks: an **Image block**
for the tutor library note editor, riding on the shared
media-asset foundation. Stored as a Tiptap atom node `libImage`
with three attrs `{ assetId, alt, caption }` — the bytes live in
Supabase Storage (private bucket `nclex-library-images`); the doc
carries only the `asset_id`. URLs are minted on demand as 1-hour
signed URLs.

### The image-persistence bug (the headline fix)

An uploaded image rendered fine, but vanished on refresh /
navigation. Root cause sat at the **React Server Action
serialization boundary**: ProseMirror builds each node's `attrs`
as a null-prototype object (`Object.create(null)`), and
`editor.getJSON()` hands that object back by reference. When the
note body crossed into a Server Action, the Flight serializer
**silently dropped the null-prototype nested objects** — the
`libImage` node arrived server-side as a bare `{ type: 'libImage' }`
with no attrs, so the saved doc had no `assetId` to reload.

Fix: deep-clone the doc through JSON in `tiptapToBody`
(`JSON.parse(JSON.stringify(doc))`) so every nested `attrs` bag is
rebuilt as a plain object before it's sent. One line; confirmed
working by Sam. Documented in CLAUDE.md → Known Workarounds.

### Upload-aware autosave gate

Sam's framing: "uploading isn't inactivity." Autosave (3s debounce)
could otherwise fire mid-upload and persist a not-yet-filled image
block. `UploadField` now emits an imperative `onUploadingChange`
signal at every entry/exit path (with an unmount safety net, since
the image block swaps the dropzone for the filled view the instant
the assetId lands). `image-block.tsx` re-broadcasts that as a
`window` CustomEvent (`LIB_IMAGE_UPLOAD_EVENT`); the note editor
counts in-flight uploads and suppresses autosave while any is
active, then a `resaveNonce` bump fires the save the moment the
upload settles.

### Pieces

- Auto-resize: large picks are downscaled in-browser (`resizeImage`)
  before upload — type-validated on the original, size-validated on
  the result, so a big original that shrinks under the 5 MB cap is
  accepted.
- Empty-state dropzone NodeView ↔ filled-image NodeView (alt +
  optional caption inputs; student read view renders the same JSON
  without Tiptap).
- New migration `20260618120000_slice_11_6a_library_images.sql`.

Files new (4): `db/migrations/20260618120000_slice_11_6a_library_images.sql`,
`lib/library/image-actions.ts`, `lib/library/image-block.tsx`,
`lib/media/resize-image.ts`. Files modified (7):
`components/media/upload-field.tsx`, `lib/library/body-tiptap.ts`,
`lib/library/note-body-editor.tsx`, `lib/library/note-editor.tsx`,
`lib/media/actions.ts`, `lib/media/types.ts`, `styles/library.css`
(+ slash-menu / nav-icon / next.config wiring).

Next: Library 11.6b (PDF / Video / Table blocks).

---

## Session — 2026-05-27 (Library Slice 11.5 follow-on — editor full build-out)

Same-day continuation of the 11.5 scaffold session. Sam tested
the freshly-shipped editor and walked through what was missing /
weak; we shipped the polish in a continuous arc rather than as
separate slices because each piece reshaped the editor's surface
and they read as one coherent pass. Three commits stacked on top
of the merged 11.5: `ec99f03`, `180ce89`, and this final commit.

### Controlled-menu refactor for the `+` buttons (`ec99f03`)

First friction Sam hit: clicking the per-block `+` or the footer
`+ Add block` button inserted a literal `/` into the doc, briefly
showed the slash menu, then dismissed it as focus moved to the
button. Two bugs in one — phantom slash + flicker.

Fix: stop faking a slash. Each `SlashItem` grows a `runAt(editor,
position)` method that inserts the block at a given position
(rather than the slash-triggered `run(editor, range)` which
deletes the slash + converts the current block). The `+` buttons
open a React-state-controlled SlashMenu that calls `runAt` on
pick. No phantom slash, no flicker, no parallel command paths
duplicated.

### Block hover + focus tint (`ec99f03`)

`@tiptap/extension-focus` adds a `has-focus` class to whichever
top-level block contains the cursor (mode: `shallowest`). CSS
tints the hovered block (5% accent) + the focused block (7%
accent; 9% when both). Matches the CD prototype's per-block
visual feedback without rendering custom NodeView wrappers.

### List bullets / numbers restored (`ec99f03`)

Tailwind's preflight clears `list-style` globally (most app
`<ul>`s are nav menus). The editor body wants markers back —
explicit `disc` / `decimal` / `circle` (nested) / `lower-alpha`
(nested) on `.lib-tiptap-body .ProseMirror`. Tight `li > p`
margin so list rows don't read loose.

### Real SVG icons (`ec99f03` + `180ce89`)

The toolbar's Unicode placeholders (`•`, `1.`, `B`, `I`) got
upgraded to SVG icons via the existing `<NavIcon>` system. Two
waves: 11 icons for the first pass (B / I / U / S / code / link
/ H2 / H3 / bullet / numbered / quote) and 12 for the second
(highlight / sub / sup / text-color / align L/C/R / undo / redo
/ clear-formatting / horizontal-rule / code-block). All paths
from Lucide (MIT) so they match the sidebar icon style.
SlashItem grows an `iconSvg?: NavIcon` field for enabled items
while disabled "coming in slice X" rows keep their emoji
placeholders.

### Toolbar expansion (`180ce89`)

Six more Tiptap extensions installed: highlight, subscript,
superscript, text-align, color, text-style. Toolbar layout
finalised:

```
[ B I U S </> 🔗 sub sup ] | [ H2 H3 ] | [ • 1. ❝ {} ─ ]
| [ highlight text-color ] | [ ⬅ ↔ ➡ ] | [ ↺ ↻ ⌫ ] | hint
```

The colour buttons each open a small swatch popover —
`lib/library/color-swatch-picker.tsx`, six brand-tone swatches +
a Remove pill, click-outside + Esc to close.

`TbButton`'s onClick signature widened to accept the mouse-event
arg so swatch triggers can read their button rect for popover
anchoring. New optional `disabled` prop for the undo/redo
buttons that gate on `editor.can().undo()` / `redo()`.

### Block tray at the editor foot (`180ce89`)

The dashed `+ Add block` button at the bottom became a horizontal
chip row of all 14 block types. Six enabled chips (Paragraph /
H2 / H3 / Bulleted / Numbered / Quote) insert at end of doc on
click via `runAt`. Eight disabled chips render dimmed with their
slice-number pill — they light up automatically as 11.6 / 11.7
/ 11.8 / 11.9 / 11.15 ship. Wraps on narrow screens.

On an empty note the tray is right under the placeholder —
inviting "pick a block to start." On a non-empty note it's the
"add another" tray.

### Inter-block "+ Add block" gap affordances (this commit)

Sam asked to replace the per-block `+` next to the drag handle
with a between-blocks insertion affordance — the dashed-line
aesthetic the footer button had, but in every gap. The mental
model: "insert here between these two specific blocks" is more
grounded than "+" floating to the side.

New `lib/library/block-gap.ts` — a Tiptap extension wrapping a
ProseMirror plugin that maintains a `DecorationSet` of widget
decorations. One widget per gap between top-level blocks.
Widget is a plain DOM button (decorations have to be DOM, not
React) that dispatches a custom `lib:block-gap-click` event with
`{ position, rect }` payload. `NoteBodyEditor`'s
`BlockEditingArea` listens on its root and opens the controlled
SlashMenu anchored to `rect` at doc `position`.

Idle: 6px transparent strip. Hover: expands to ~28px with a
dashed accent line + "+ Add block" pill in the middle. Click →
menu opens at that gap.

The per-block `+` was removed; only the drag handle stays in the
left-margin cluster.

### Shared `usePopoverPosition` hook (this commit)

Final fix: the SlashMenu + ColorSwatchPicker were positioned at
a fixed `anchor.bottom + 6` regardless of viewport. Near the
page bottom the menu extended off-screen and got clipped.

`lib/library/use-popover-position.ts` is the shared hook:
- Default: position below anchor.
- If popover height + gap exceeds space below → flip and position
  above.
- If popover doesn't fit either side → use the larger side with a
  computed `max-height` so the popover's own `overflow-y: auto`
  handles internal scroll (slash menu already has `max-height:
  360px`).
- Horizontal: align to anchor's left edge, but clamp right if it
  would overflow the viewport.

**Implementation gotcha** the first attempt walked into: storing
the computed style in `setState` from a `useLayoutEffect` causes
a re-render → another effect run → another `setState`, infinite
loop ("Maximum update depth exceeded"). Fixed by mutating the
popover's DOM `style` directly inside the layout effect — no
React state, no feedback loop. Browser paints with the correct
position because `useLayoutEffect` runs before paint.

### Files

**New (4):** `lib/library/block-gap.ts`,
`lib/library/color-swatch-picker.tsx`,
`lib/library/use-popover-position.ts`,
12 icon cases in `components/nav/shared/nav-icon.tsx`
(spread across the first + second wave).

**Modified (heavily):** `lib/library/note-body-editor.tsx`
(every section — extensions, toolbar, block tray, inter-block
gap listener, drag-handle cluster trimmed), `lib/library/slash-menu.tsx`
(`runAt` + `iconSvg` field + popover-position hook),
`lib/library/color-swatch-picker.tsx` (popover-position hook),
`lib/nav/types.ts` (23 new `NavIcon` cases),
`styles/library.css` (extensive — toolbar groups, swatch
popovers, block tray chips, inter-block gap, block hover/focus
tint, list bullets, new icon styles), `package.json` +
`package-lock.json` (7 new Tiptap deps total across the
follow-on).

**Doc updates:** `BUILD_LIST.md` last-shipped + `⏭` pointer
now Library 11.6, `SESSIONS.md` count bumped to 69, this entry.

### What's next

`BUILD_LIST.md` `⏭` is now **Library 11.6** (visual blocks:
Image / PDF / Video / Table + Supabase Storage signed-URL
pipeline). Standing alternate: Payments **5.3** per the
alternate-features rotation. As 11.6 / 11.7 / 11.8 / 11.9 /
11.15 ship, their slash-menu rows + block-tray chips
automatically light up — they're already wired as disabled
placeholders carrying their slice numbers.

---

## Session — 2026-05-27 (Library Slice 11.5 — Tiptap editor scaffold, 11.5a + 11.5b)

Big library slice. Two sub-slices shipped together in one session
— 11.5a built the Tiptap foundation (rich-text editing + autosave
+ two-tabs save guard); 11.5b layered the block UX from the CD
prototype (slash command, drag handles, placeholder, CD visual
treatment, rail collapse). Sam chose to combine them so he'd test
the full Notion-like experience in one pass rather than an
intermediate plain-Tiptap state.

### 11.5a — foundation

**Install.** `@tiptap/react@3.23.6` + `@tiptap/starter-kit` +
`@tiptap/extension-link` + `@tiptap/extension-underline`. Tiptap 3
bundles link + underline into StarterKit by configuration, so
explicit deps are kept for the underlying packages but the
extensions are registered via `StarterKit.configure(...)`.

The first npm install hit two Windows-specific issues already
documented in CLAUDE.md: TAR_ENTRY_ERROR warnings (dev server
holding next/dist files open — fixed by stopping dev server first)
and the lightningcss optional-dependencies bug (copy the binary
from `node_modules/lightningcss-win32-x64-msvc/` into
`node_modules/lightningcss/`).

**Body shape adapter** (`lib/library/body-tiptap.ts`). Stores
Tiptap's native `{ type: 'doc', content: [...] }` directly in the
JSONB column going forward. Legacy 11.2b notes (array of one
paragraph block) migrate to Tiptap's shape on load — `bodyToTiptap`
recognises both shapes; `tiptapToBody` writes Tiptap's verbatim.
Also exports `deriveOutlineFromDoc` which walks the doc for H2/H3
nodes and returns a flat list for the right rail outline (no
@tiptap/* import — module is safe for future server-side use).

**NoteBodyEditor** (`lib/library/note-body-editor.tsx`). Client
component wrapping `useEditor` with StarterKit configured for H2/H3
only. Provides the always-visible toolbar above the ProseMirror
element. Body has no internal scroll — the `.product-content`
scroll (slice 2.9 chrome) carries the page.

**Server `version_id` guard.** `updateNoteAction` now expects
`expected_version_id` in the input. The UPDATE statement is
conditional (`.eq('version_id', expected)`); a mismatched row
returns null. A follow-up probe SELECT distinguishes "row missing"
from "version moved on", returning `{ ok: false, conflict: true }`
in the latter case. The note-editor surfaces a "this was saved in
another tab" toast and stops autosaving when conflict fires.

**Autosave + savedState baseline.** Two related refactors that
made the loop stable:

- **savedState.** A separate piece of state alongside `note` (the
  SSR snapshot) tracks the most-recently-committed field values.
  `isDirty` compares current state to `savedState`, not `note`.
  After a successful save, `setSavedState(...)` commits the new
  baseline so `isDirty` flips back to false without a refetch.
- **Refs for closures.** `fieldsRef` and `saveStateRef` mirror
  state values that the autosave effect reads at fire time. This
  keeps the effect's dep list to *edit triggers only* (title,
  subtitle, description, folderId, pillars, tags, bodyEditVersion,
  canSave); putting `saveState` in deps would cause the timer to
  reset every time the badge state changed, defeating the
  debounce.

The Save button is gone. Browser `beforeunload` still guards close-
tab when dirty; `attemptLeave` still surfaces `DiscardConfirm` for
in-app breadcrumb navigation.
...
`slash-menu.tsx`). Custom Tiptap extension built on
`@tiptap/suggestion`. Trigger char `/`, allowSpaces false,
startOfLine false. Items list is the full 12-block taxonomy
(`SLASH_ITEMS`) in 4 groups; `filterSlashItems(query)` does the
loose substring match. Render path uses Tiptap's `ReactRenderer`
to mount `<SlashMenu>` portaled at `document.body`, positioned
absolutely via the suggestion's `clientRect` callback.

The 6 text-block types are enabled with `run: (editor, range) =>
editor.chain().focus().deleteRange(range).<command>().run()` —
deleteRange removes the slash + filter text before inserting. The
6 non-text types (image / pdf / video / table / callout /
drug_card / lab_values / embedded_questions) carry `comingIn:
'11.6'` etc. and render disabled with a slice-number badge.
Keyboard nav skips disabled rows.

**Per-block drag handles.** `@tiptap/extension-drag-handle-react`
mounted as a React component sibling to `<EditorContent>` inside
`<div className="lib-tiptap-body-wrap">`. The library's extension
auto-positions its host element to the left of the currently-
hovered block; we render a `⋮⋮` glyph inside it and style hover
states.

**Placeholder.** `@tiptap/extension-placeholder` writes
`data-placeholder` on every empty node; CSS pseudo-element renders
it inline. Per-node text ("Heading 2" / "Heading 3" for empty
headings) via the extension's per-node callback.

**Simplified toolbar.** Dropped the Paragraph/H2/H3 dropdown from
11.5a — H2 and H3 are now direct toolbar buttons matching the CD
prototype's flat shape. Right edge carries a "Type / for blocks"
discoverability hint with a `<kbd>` chip.

**CD visual treatment.** H2 26px serif accent-coloured weight 600
with 22px top margin; H3 20px serif accent-coloured; paragraph
15px line-height 1.65 + `text-wrap: pretty`. Quote keeps the 3px
accent left-border. Inline code keeps the tinted chip.

**Rail collapse toggle.** `»` button in the Status section head
(via new `.lib-rail-section-head` flex row); collapses the rail
entirely via `.is-rail-collapsed` on the grid. Restore button
(`«`) renders absolutely positioned at top-right of the main pane
when collapsed. Preference persists in
`mynclex.library.editor.rail-collapsed`.

**Outline live updates.** Right rail's Outline section reads from
the live Tiptap doc (via `deriveOutlineFromDoc`) on every body
edit. H3 entries indent + mute relative to H2.

### What's queued

Three pieces deferred to a follow-on polish slice once Tiptap
proves out in real use:
- Block kebab menu (Delete / Duplicate / Convert) — drag handle
  covers reorder; conversion runs through slash today.
- BroadcastChannel pre-warning when two tabs open the same note —
  the version_id guard already catches the bad save.
- "Attached to N programmes…" edit-propagation warning — needs
  the attachments table populated by 11.11.

### Files

**New (4):** `lib/library/body-tiptap.ts`,
`lib/library/note-body-editor.tsx`,
`lib/library/slash-command.ts`,
`lib/library/slash-menu.tsx`.

**Modified (6):** `lib/library/types.ts` (add
`expected_version_id` to `LibraryNoteUpdateValues`),
`lib/library/actions.ts` (conditional UPDATE + conflict probe +
`updated_at` in result), `lib/library/note-editor.tsx`
(full rewrite — autosave + savedState + version_id ratchet +
NoteBodyEditor swap + outline-from-doc + rail-collapse),
`styles/library.css` (Tiptap shell + sticky toolbar +
ProseMirror block styling + slash popover + drag handle +
rail-collapse), `package.json` + `package-lock.json` (7 new
Tiptap deps).

**Doc updates:** `BUILD_LIST.md` last-shipped + ⏭ pointer (now
Library **11.6** — visual blocks); `docs/product-plan/tutor-library.md`
slice 11.5 flipped to ✅ with combined-shipped note;
`SESSIONS.md` count bumped to 68; this entry.

### What's next

`BUILD_LIST.md` ⏭ now points to **Library 11.6** (Standard visual
blocks: Image / PDF / Video / Table + Supabase Storage signed-URL
pipeline + auto-resize). Standing alternate: Payments **5.3** per
the rotation rule.

---

## Session — 2026-05-27 (Slice 2.9 — locked shell + railed sidebars + sidebar user bar)

Chrome polish slice. Sam flagged three things after the Library
Overview slice landed: (1) the sidebars grow with the page height
and slide out of view when content is long; (2) we want to be able
to collapse them to an icon-only strip — same idea as the library's
internal rail, but applied to the global sidebars; (3) something
sitting at the bottom of each sidebar that opens a future settings
/ account popup. Worked through the design over a short discussion,
then shipped as one slice.

### Layout — locked viewport

`.shell-root` becomes `height: 100dvh; overflow: hidden` and stays
a flex column. The topbar is now a non-shrinking row at the top
(dropped `position: sticky` — meaningless in a locked layout).
`.shell-body` fills the remaining height with `flex: 1; min-height:
0; overflow: hidden`. Inside it, `.product-layout` switches from a
fixed `220px 1fr` grid to `auto 1fr` (the sidebar's `<SidebarFrame>`
drives its own width — 220px expanded, 56px railed), with `flex: 1;
min-height: 0; overflow: hidden` so it fills the body. Only
`.product-content` carries `overflow-y: auto`, making it the single
scrollable region.

The picker (no sidebar) has its own scroll behaviour — flex column,
`flex: 1; min-height: 0; overflow-y: auto`, with the footer pinned
to the bottom of its scroll via `margin-top: auto`.

### Sidebar collapse — `<SidebarFrame>`

New `lib/`-style component at `components/nav/shared/sidebar-frame.tsx`.
Client component. Wraps every audience sidebar without replacing
its `<aside>` — just adds the column chrome around it.

State: one global localStorage key `mynclex.sidebar.railed`. Sam's
choice — collapsing on the tutor global sidebar carries to admin,
to the student bank, to every other sidebar everywhere. The frame
listens for both same-tab events (custom event
`mynclex.sidebar.railed-changed` so multiple `<SidebarFrame>`
mounts on one page stay in sync) and cross-tab `storage` events.

Shape:
- Top — `«` / `»` collapse toggle button (36px tall, bottom border,
  hover tint).
- Header slot — optional, used by student programme/cohort shells
  for the "Switch programme" button (previously wrapped in a
  `.sidebar-column` div which is now retired).
- Body — the audience sidebar's `<aside>`. The inner aside drops
  its own bg/border (frame owns those) and gains `flex: 1;
  overflow-y: auto` so long nav lists scroll internally.
- User bar slot — pinned to the foot.

Railed (56px) state: CSS hides `.nav-label`, `.nav-sub-list`,
`.caret`, `.sidebar-header`, `.sidebar-context`, `.sidebar-footer`,
`.sidebar-switcher-label`, `.sidebar-switcher-chev`,
`.sidebar-user-bar-name`, `.sidebar-user-bar-chev`. Nav items
centre their icons. Width transitions over 0.18s. Full viewport
height stays the same as expanded — Sam's specific call to
distinguish the global rails from the library's inner rail (which
is short because it lives inside a page; global rails are
structural chrome and stay tall).

### Sidebar user bar — `<SidebarUserBar>`

New `components/nav/shared/sidebar-user-bar.tsx`. Placeholder for
this slice — clicking the bar opens a popover above with the
user's name + email and "Settings, account and more coming soon"
stub. Settings / account / role-switching / sign-out land later
when Sam decides what goes in.

The topbar's existing user menu + role chip stay where they are —
Sam decided to keep both for now; "if later we decide what to put
in the place holder then we will know."

Expanded state: 28px avatar circle with initials + name + ▾ chev,
full-width row at the bottom of the frame. Railed: just the avatar,
centred. Popover anchors above the bar in expanded mode; in railed
mode it anchors to the right of the rail (so it doesn't clip).

### Footer treatment

Option B per the discussion — the footer lives inside the content
scroll (not always visible, not pinned to the bottom of the
viewport). `<AppShell>` stopped rendering `<Footer />`. Each
audience shell now renders `<Footer />` as the last element inside
`<main className="product-content">`. The picker renders it at the
end of `.picker`.

Public pages (`app/(public)/`) are untouched — they have their own
`<PublicNav />` + `<PublicFooter />` and bypass `<AppShell>`
entirely, so landing / pricing pages still document-scroll
normally with their own marketing-sized footer. Sam wanted the
ability for public and authenticated footers to diverge; they
already do, and now they can grow independently.

### Files

**New (2):** `components/nav/shared/sidebar-frame.tsx`,
`components/nav/shared/sidebar-user-bar.tsx`.

**Modified (12):** `styles/shell.css`, `styles/nav.css`,
`styles/student-curriculum.css`, `components/shell/app-shell.tsx`,
`components/nav/admin/admin-shell.tsx`,
`components/nav/tutor/global-shell.tsx`,
`components/nav/tutor/programme-shell.tsx`,
`components/nav/tutor/cohort-shell.tsx`,
`components/nav/student/programme-shell.tsx`,
`components/nav/student/cohort-shell.tsx`,
`app/(app)/student/bank/layout.tsx`,
`app/(app)/student/picker/page.tsx`.

**Doc updates:** `BUILD_LIST.md` last-shipped block,
`SESSIONS.md` session count bumped to 67, this entry.

### Same-session follow-on — sticky footer on short pages

Sam tested and flagged: on short pages (programme overview,
students, etc.) the footer was glued right under the content
with the bottom half of the viewport empty. Fix is one CSS rule —
make `.product-content` a flex column and give the trailing
`.shell-footer` `margin-top: auto`. On short pages the footer
slides to the bottom of the scroll area; on tall pages it sits
below content as normal flow. The picker already had this
pattern.

### What's next

Sam is taking the per-program / per-cohort sidebar customisation
question to Claude Desktop / Claude Design for further ideas, then
we'll pick what's adoptable from their suggestions. Standing build
candidates remain Library 11.5 (Tiptap editor scaffold) and
Payments 5.3 (alternate-features rotation).

---

## Session — 2026-05-27 (Library — Overview + system Views + sidebar polish)

Same worktree, third commit of the session. Built the P2 slice
locked yesterday — Library Overview dashboard at `/tutor/library`
+ three system views + sidebar Overview entry + three small
sidebar polish items Sam flagged in the same session.

### Data plumbing

**`fetchAllLensRowsForTutor`** — new helper, wrapped in React's
`cache()`. Returns every lens row the tutor owns, ordered by
`updated_at` desc. Called by `getLibraryLensCounts`,
`getLibraryOverviewStats`, and `getNotesForView` — multiple
consumers in one RSC render share a single round trip via the
cache wrapper. Same PostgREST embed shape as `getNotesForTutor`
(folders + shelf memberships + attachments(count)) but unfiltered.

**Three new exported queries** in `queries.ts`:
- `getLibraryLensCounts()` — returns `{ view: { all, drafts,
  used_nowhere }, pillars: Record<NclexPillar, number> }`.
  Called on every page render to light up the sidebar Views +
  Pillars lens entries.
- `getLibraryOverviewStats()` — returns the dashboard data:
  total_notes / folders / shelves / drafts / used_nowhere /
  pillar_counts / recent (last 5). Parallelises three reads
  (notes + folder count + shelf count via `head: true`).
- `getNotesForView(key)` — switches on key and filters the
  all-notes array. `all-notes` returns everything; `drafts`
  filters `!is_published`; `used-nowhere` filters
  `used_in_count === 0`.

**Three new exported types** in `types.ts`:
`LibraryViewKey` (the three wired view keys),
`LibraryViewCounts`, `LibraryOverviewStats`.

### Dashboard

New `lib/library/library-overview.tsx`:
- Stat cards row — 5 numbers across (responsive: 5 → 3 → 2
  cols).
- Recent activity — `<ol>` of the last 5 edited notes,
  clickable straight into the editor; each row carries title +
  "Folder · edited X ago" meta line.
- Pillar coverage — horizontal bar per pillar, all 8 rendered
  even when count is 0. Bars scaled to the busiest pillar
  (`width = (count / maxPillarCount) * 100%`), so coverage gaps
  are visible at a glance.
- Quick links — chip row routing to All notes / Drafts /
  Used nowhere / All folders / All shelves.

### System view pages

New `lib/library/notes-view.tsx`. Single component, switches on
viewKey for the title / sub-line / empty-state copy. Note rows
delegate to `<NoteLensRow>` (no per-view styling). View-specific
empty-state copy: "No drafts — everything is published." /
"Every note is doing some work — none of them are orphans."
etc.

### Routing

`page.tsx` rewritten with 4-way scope precedence:
`?shelf=` > `?view=` > `?folder=` > no scope (overview).
Branches dispatch independent fetches in `Promise.all`. Lens
counts always fetched so the sidebar lights up regardless of
scope. Detail-scope and view-scope branches skip the heavier
shelves-with-notes / folder-notes queries when they're not on
that branch.

### Sidebar improvements (same-session polish)

Sam flagged four small UX gaps after testing the overview:

**Permanent Overview entry.** New 🏠 Overview row sits at the
top of the sidebar, above the lens sections. Active when no
scope is set (selected + shelfSelected + viewSelected all
null). Railed mode shows just the glyph with a "Overview"
tooltip. Distinct from any lens — it's a destination, not a
filter. Style: `.lens-home` block in `styles/library.css`.

**Lens header glyphs (expanded mode).** Each lens section
header now renders `SECTION_RAIL_GLYPH[lens]` in front of the
label — ☰ Views / 📁 Folders / 📚 Shelves / ◆ Pillars /
# Tags. Visual consistency with the railed view, which already
uses these glyphs as the only affordance.

**Railed icons are clickable.** Three rules:
- Views / Folders / Shelves → Link to their default landing
  (`?view=all-notes` / `?folder=all` / `?shelf=all`).
- Pillars / Tags → `<button>` that calls `onExpandRail` (which
  triggers the rail's expand state). No destination is wired
  yet for these lenses; the planning doc has filter chips for
  pillars and tags rather than a dedicated landing page.
- A new `SECTION_RAIL_HREF: Partial<Record<LensKey, string>>`
  constant maps the navigable lenses to their default
  destinations — when an "All tags view" ships later
  (deferred follow-on), it becomes a one-line swap in this
  map.

**Stronger lens dividers.** Section divider colour bumped from
`color-mix(in srgb, var(--border) 60%, transparent)` to
`var(--border-strong)`; top padding bumped 10 → 12px; top
margin bumped 6 → 10px. More breathing room and a more
obvious separation between Views / Folders / Shelves /
Pillars / Tags.

### Files

**New (2):** `lib/library/library-overview.tsx`,
`lib/library/notes-view.tsx`.

**Modified (5):** `lib/library/types.ts`, `lib/library/queries.ts`,
`lib/library/home-shell.tsx` (Overview entry + lens-header
icons + railed Links + main-pane view branch),
`app/(app)/tutor/library/page.tsx` (4-way scope dispatch),
`styles/library.css` (SLICE 11.4 follow-on P2 block + sidebar
polish).

**Doc updates:** `BUILD_LIST.md` last-shipped + ⏭ pointer
(now Slice 11.5 — Tiptap editor scaffold);
`docs/product-plan/tutor-library.md` queued slice ✅ + new
deferred follow-on for All tags view; `sessions/2026-05.md`
this entry; `SESSIONS.md` session count bumped to 66.

### What's next

`BUILD_LIST.md` ⏭ pointer is now **Slice 11.5 — Tiptap editor
scaffold**. With Overview + Views shipped, the library has its
foundation: a real home, three browse views, consistent rows
everywhere, sidebar shortcuts to everything. The rich block
editor is the next big arc. Or rotate per the alternate-features
rule — Payments 5.3 is the live alternate.

---

## Session — 2026-05-27 (Library — shared NoteLensRow + editor On shelves rail; P3+P4 bundle)

Continued on the same `claude/distracted-saha-a1ca83` worktree —
session crossed midnight while building, so this is logged
2026-05-27. Built the bundled P3+P4 slice locked yesterday: a
single `<NoteLensRow>` component used by every full-width note
row + a read-only "On shelves" section in the editor right rail.

### Why bundled

P3 (consistent note card across surfaces) and P4 (editor shows
shelf membership) need the same data: `shelf_memberships:
{ shelf_id, title, color }[]` on the note. Two slices with the
same enrichment pass would have done the work twice. One slice =
one query enrichment + both surfaces.

### Data plumbing

**Canonical `LibraryNoteLensRow` projection** in `types.ts`:
note_id + folder_id + folder_name + title + subtitle +
description + pillars + tags + shelf_memberships
(LibraryShelfPip[]) + is_published + visibility_mode +
updated_at + used_in_count. `LibraryNoteListRow` becomes a thin
alias of this. `LibraryShelfDetailNote` extends it (adds
`position`). `LibraryNoteForEdit` gains `shelf_memberships` so
the editor rail can render.

**Shared embed helpers** at the top of `queries.ts`:
`extractFolderName`, `extractShelfPips`, `extractAttachmentCount`
each take the raw PostgREST embed shape (which may be `T | T[] |
null` depending on FK direction) and return the projection
field. Reused by all three queries to flatten the embeds
consistently.

**`getNotesForTutor`** rewritten — one PostgREST query with
embeds: `folders(name)` + `shelf_memberships(shelves(shelf_id,
title, color))` + `note_attachments(count)`. Returns
LibraryNoteLensRow[].

**`getShelfDetail`** split into two queries. The original 11.4
shape was a deep PostgREST embed (shelves → memberships → notes
→ memberships(count) for the other-shelves derivation). With the
new shape we'd need shelves → memberships → notes → memberships
→ shelves — a self-referencing cycle. PostgREST can handle it
with `alias:table_name` syntax but it's fiddly. Cleaner: two
round trips — query 1 reads the shelf + ordered (position,
note_id) pairs; query 2 batched-fetches the lens-row data for
those note IDs via `IN (...)`. Merge in JS using a Map<id, row>.
Same total round-trip cost as a folder list; sub-millisecond
overhead on dev datasets.

**`getNoteForEdit`** picks up the same shelf_memberships embed.

### NoteLensRow component

New `lib/library/note-lens-row.tsx`. Props: `note`,
`numberPrefix?` (renders `1.` tabular-numerals on the left, used
by shelf detail), `excludeShelfId?` (filters one shelf out of
the pip cluster so shelf-detail rows don't repeat the page-scope
pip). The row IS the `<Link>` to the editor. External
affordances (reorder ▲▼, hover ✕) live as sibling absolute-
positioned elements inside the wrapping `<li>`, with
`preventDefault + stopPropagation` so they don't fire the row's
navigation.

Right column three-stack:
- Pub/Draft pill — always there
- `↳ used in N` — hidden when zero (no signal to show)
- `edited Xd ago` — always there

Meta row order: 📁 folder · pip cluster · pillar chips · #tags.
Shelf pips are 8px coloured circles with the shelf's identity
colour as background and a tooltip on hover.

### Editor "On shelves" rail

New section in `note-editor.tsx`, between Status and Outline.
Renders `note.shelf_memberships` as a list of clickable rows
(coloured pip + shelf title → `?shelf=<id>`). Empty state:
"Not on any shelf yet. Add this note to a shelf from the All
Shelves carousel or any shelf's detail page." Per the locked
design — **no edit affordance in v1**; add/remove happens from
the shelf-side flows (carousel `+ Add to shelf`, shelf-detail
`+ Add notes`).

### Side-quest: formatRelative hoisted

The "edited X ago" copy on every row needs the same relative-
time formatter the editor's save badge uses. Hoisted out of
`note-editor.tsx` into `lib/library/format.ts` as a small public
helper.

### Migrations of existing surfaces

- **`<NotesList>` (folder scope)** — bespoke row JSX replaced by
  `<NoteLensRow note={n} />`. Lost the standalone description-or-
  subtitle fallback function call (now inside NoteLensRow), the
  inline-tag separator dot (no longer needed when chips render
  uniformly).
- **`<ShelfDetail>`** — the inline row markup replaced by
  `<NoteLensRow note={note} numberPrefix={ordinal}
  excludeShelfId={shelfId} />`. The wrapping `<li>` keeps the
  shelf-accent left-edge bar (via a descendant CSS selector that
  applies the `--shelf-accent` colour to the lens row inside the
  wrap) + the floating tool group.
- **Carousel** untouched — its compact card is a different
  visual container per the locked design (horizontal scroll
  demands it).

### CSS

New SLICE 11.4 follow-on block appended to `styles/library.css`:
- `.lib-note-lens-row` — the row Link itself
- `.lib-note-lens-num` — tabular-numerals number column
- `.lib-note-lens-main` — main content column
- `.lib-note-lens-state` — three-stack right column
- `.lib-state-used` + `.lib-state-edited` — the two extra pills
- `.lib-shelf-pip` + `.lib-shelf-pip-cluster` — inline pip cluster
- `.lib-shelf-detail-row-wrap .lib-note-lens-row` — descendant
  selector for the shelf-accent left bar + the right-padding
  reservation for the floating tools
- `.lib-rail-shelves` + `.lib-rail-shelf-link` + `.lib-rail-shelf-title`
  — editor rail's "On shelves" section rows

Old `.lib-note-row` + `.lib-shelf-detail-row*` styles left in
place — dead code but harmless; cleanup can come in a polish
pass once we're sure nothing else references them.

### Files

**New (1):** `lib/library/note-lens-row.tsx`.

**Modified (7):** `lib/library/types.ts`, `lib/library/queries.ts`,
`lib/library/format.ts`, `lib/library/notes-list.tsx`,
`lib/library/shelf-detail.tsx`, `lib/library/note-editor.tsx`,
`styles/library.css`.

### What's next

`BUILD_LIST.md` ⏭ now points at **Library Overview + system
Views** (the P2 slice locked yesterday). `/tutor/library` (no
scope) becomes a dashboard; `?view=<key>` wires All notes /
Drafts / Used nowhere; Recent stays disabled until visit-tracking
ships. Tiptap editor scaffold (was the original ⏭) bumps after
that. Or rotate per the alternate-features rule — Payments 5.3
is the live alternate.

---

## Session — 2026-05-26 (Post-11.4 polish + 3-slice design lock)

Same worktree as the 11.4 session — Sam reviewed the shipped
shelf-detail view and surfaced four polish points he wanted to
discuss before moving on. The session split into a small polish
commit (point 1 — folder kebab + editor edit-cue), three queued
slices (points 2–4 design-locked), and a deferred follow-on
(note deletion).

### Point 1 — folder kebab + editor edit-cue (commit pending)

Two gaps Sam flagged from 11.4 testing:

- **Folders had no edit/delete UI.** Shelves got the kebab pattern
  in 11.3a, folders missed it entirely. No `editFolderAction`,
  no `deleteFolderAction`. The only way to "edit" a folder was to
  delete and recreate.
- **Editor title / subtitle / description didn't read as
  editable.** They're styled as large serif title, italic
  subtitle, small one-line description with `border: none;
  background: transparent` — visually display-text. A new tutor
  reading the editor wouldn't know they could click and type.

Resolved both in one slice:

**Folder kebab + Edit / Delete.** `<FolderRows>` refactored to a
per-row `<FolderRow>` sub-component carrying the hover-revealed
`⋮` button + popover menu pattern from `<ShelfRow>` (Edit /
Delete entries, click-outside + Escape close). `<NewFolderModal>`
refactored to a discriminated `variant: { mode: 'create' } |
{ mode: 'edit'; folder }` mirroring `<NewShelfModal>` —
pre-fills from the folder, dup-check excludes self, mode-aware
copy + button label. New `editFolderAction` (auth + dup-check +
UPDATE, mirrors editShelfAction).

**Folder delete with orphan-to-root semantics.** Sam picked this
over "refuse delete when non-empty" + "cascade delete with
type-DELETE" — notes are precious (body content + shelf
memberships + programme attachments + visibility settings) and
the typical tutor intent is to clean up the folder layer
without losing the contained notes. New `deleteFolderAction`
runs two writes in sequence: UPDATE notes SET folder_id = NULL
WHERE folder_id = ? (returning the affected rows for the count
copy), then DELETE folder. `folder_id` is already nullable on
`nclex_tutor_library_notes` per the 11.1a migration so no schema
change. New `<DeleteFolderConfirm>` is a simple yes/no — not
type-to-confirm, since nothing is destroyed — with note-count-
aware copy ("The N notes inside will move to Root — body
content, shelf memberships, programme attachments and
visibility settings are all kept").

**Editor edit-cue.** Each of title / subtitle / description
wrapped in a `.lib-editor-editable` div with a hover- and
focus-within-revealed `✎` icon at the right edge + a subtle
accent tint (`color-mix(in srgb, var(--accent) 5%,
transparent)`) on the field. Pencil is a `<span aria-hidden>`
with `pointer-events: none` so clicks fall through to the input
behind. Wrap reserves 30px right padding for the icon; the
inputs' existing margin-bottoms drive vertical rhythm. Solves
"looks like display text" without restructuring the editor or
moving fields into the rail.

**Files new (1):** `lib/library/delete-folder-confirm.tsx`.
**Files modified (5):** `actions.ts`, `new-folder-modal.tsx`
(discriminated mode), `folder-rows.tsx` (kebab + edit/delete
state), `home-shell.tsx` (variant prop), `note-editor.tsx`
(editable wraps + pencil) + `styles/library.css`
(`.lib-editor-editable` block).

### Points 2 / 3 / 4 — design lock (3 queued slices)

After point 1 was built, Sam raised three more discoverability
gaps. All three got design-locked this session and queued ahead
of slice 11.5 (Tiptap editor scaffold). Build order settled:
**point 1 (built) → P3+P4 bundled → P2.** Reason for that order:
P3 and P4 share the same `shelf_memberships` projection on the
note (cheaper bundled); P2's view pages then reuse
`<NoteLensRow>` from P3.

**Point 2 — Library Overview + system Views.** `/tutor/library`
(no scope) is currently the generic `<EmptyState>` ("Your
library is empty… start with a folder or jump straight to a new
note") — regardless of whether the tutor actually has content.
Sam wanted a dashboard instead. v1 carries: 5 stat cards (Total
notes · Folders · Shelves · Drafts · Not yet in any programme
unit) + Recent activity (last 5 edited, clickable into the
editor) + Pillar coverage (horizontal bar per pillar showing
relative count — surfaces gaps) + Quick links to each system
view. The 4 disabled Views lens entries get wired in the same
slice: **All notes**, **Drafts** (`is_published = false`), and
**Used nowhere** (zero programme attachments) ship; **Recent**
stays disabled until visit-tracking ships (needs
`last_visited_at` on `nclex_library_note_state` which no UI
writes today). URL scheme: `?view=<key>` where key ∈ {`all-notes`,
`drafts`, `used-nowhere`}.

**Point 3 — Note card consistency.** Three different lens-row
shapes in the codebase today: `.lib-note-row` (folder list),
`.lib-shelf-detail-row` (shelf detail — new in 11.4), and
`.lib-shelf-card` (carousel). The two row variants share the
same content philosophy but were built independently — drifts
over time. Decision: extract a single `<NoteLensRow>` used by
every full-width row context (folder list + shelf detail + the
three Views + future custom views), and keep the carousel
distinct because horizontal scroll demands the compact card.
Canonical fields per the planning doc (lines 211–216): title +
inline subtitle + description-or-body-excerpt fallback + meta
line carrying 📁 folder · coloured shelf pip(s) · pillar chips
· #tags, right column carrying Pub/Draft pill + ↳ used-in N +
edited-relative-time. **Shelf membership renders as pips, not a
count badge** — one coloured dot per shelf carrying that shelf's
identity colour with title tooltip. Folder chip always shown
(slight redundancy in folder scope is fine and never confusing).

**Point 4 — Editor "On shelves" rail.** The editor's right rail
has Status / Outline / Embedded questions / Used in (programme
attachments) / Guards — silent on shelf memberships entirely.
Sam wanted the editor to show "this note is on Foundational SATA
pack + Drug deep dives." Decision: read-only rail section under
Status, one row per shelf (coloured pip + title, clickable into
`?shelf=<id>`). **No edit affordance in the editor for v1** —
add-to-shelf already happens from the shelf-side flows (carousel
`+ Add to shelf`, shelf-detail `+ Add notes`); a third entry
point would duplicate UI for a flow that's already complete.

### Deferred follow-on — note deletion

Sam asked mid-session whether note deletion was built. Honest
answer: no — no `deleteNoteAction` exists, no UI surface
(`<NotesList>` rows have no kebab, the editor toolbar has no
Delete button). DB layer is ready:
`nclex_tutor_library_note_attachments.note_id` is `ON DELETE
RESTRICT` (refuse delete when the note is attached to a
programme unit); `_shelf_memberships.note_id` is `ON DELETE
CASCADE` (memberships drop silently). Sam queued for later —
captured as a deferred follow-on in both `BUILD_LIST.md` and
the planning doc. Natural pairing: the note-card-consistency
slice (which is touching the row components anyway) or Publish
(11.10).

---

## Session — 2026-05-26 (Library Slice 11.4 — shelf detail view)

Fresh worktree `claude/distracted-saha-a1ca83`. Sam picked Library
11.4 over the alternate-features rule's natural rotation to Payments
5.3 — five library slices in a row now, but 11.4 is the natural
follow-on that activates URLs the sidebar has been routing to a
stub since 11.3a, so the close-the-loop logic won.

### Scope locks (before code)

Two open questions answered up front via `AskUserQuestion`:

- **Ship reorder in 11.4 (Sam's pick).** The planning doc calls
  for "numbered notes in master shelf order"; numbered ordering is
  only meaningful if the curator can set it. Up/down arrows ship
  alongside the detail page, not as a follow-on slice.
- **Defer the filter bar (pillar / tag chips) inside a shelf.**
  Most v1 shelves are small; chip filtering inside a 10-row list
  adds chrome for little payoff. The planning doc's main pane
  shape calls for it, but in practice the lens row's inline
  metadata is enough — slot when a real tutor with a big shelf
  asks.

### What shipped (commit pending)

**Routing rewire.** `?shelf=<uuid>` becomes a real destination.
`<ShelfRows>` per-shelf href is now `?shelf=${shelf_id}` (was
`?shelf=all` since 11.3a as a placeholder). `is-active` class +
`aria-current="page"` light up when the URL matches. `?shelf=all`
continues to render the carousel; unknown UUID surfaces a
`<ShelfNotFound>` empty state mirroring `<FolderNotFound>` from
11.2a.

**`page.tsx` — 3-way branch.**
- `shelfSelected = null` → no shelf scope, lean shelf list only.
- `shelfSelected = 'all'` → carousel (heavy fetch with note embeds).
- `shelfSelected = <uuid>` → per-shelf detail.

The lean shelf list (`getShelvesForTutor`) is fetched
unconditionally now — one cheap round trip — and the
"derive lean from heavy" branch from 11.3b's `page.tsx` is gone.
Eligibles pre-fetch only for the relevant case (every shelf for
the carousel, just the focused shelf for the detail view).

**`getShelfDetail(shelfId)`.** New query. Single round trip via
PostgREST nested embed: shelves → memberships(position) → notes →
folder(name) + memberships(count). `other_shelf_count` derives in
JS as `total - 1` (the count includes this shelf's own
membership). RLS-gated `.maybeSingle()` returns null for both
"doesn't exist" and "not yours" — same not-found surface either
way. The double-membership embed works because PostgREST follows
the two FKs unambiguously (the outer is `shelves → memberships`,
the inner is `notes → memberships`).

**`reorderShelfMemberAction(shelfId, noteId, direction)`.**
Two-UPDATE swap, not a positional re-write. There's no UNIQUE on
`(shelf_id, position)` in the schema (PK is `(shelf_id,
note_id)`), so the intermediate state can't collide and the swap
writes directly. Pulls the ordered membership list once to find
the neighbour, then writes both `position` columns. Boundary
case (top row ▲ / bottom row ▼) returns ok:true no-op so the UI
doesn't need to disable the button optimistically — the action
absorbs it.

**`<ShelfDetail>`.** New component at `lib/library/shelf-detail.tsx`.
Layout:
- Header — crumb (Library / Shelves / `<title>`) + title row
  (14px identity dot + 22px title + `Shelf · curated` lens
  badge) + sub-line (count · tagline) + optional description
  paragraph below + `+ Add notes` primary button on the right.
- Numbered `<ol>` — each `<li>` carries a 3px shelf-coloured
  accent bar on the left, a tabular-numerals number column
  (`1.` `2.` `3.`), the lens-row content (title + subtitle line,
  description-or-subtitle fallback, inline meta — 📁 folder ·
  📚 +N other-shelf · pillar chips · #tags), and a Pub/Draft
  state pill on the right.
- Floating tool group — absolutely positioned over the right
  edge of each row, hover- and focus-within–revealed (so
  keyboard nav surfaces it too). Up arrow + down arrow + ✕
  remove. `preventDefault + stopPropagation` so the row's
  outer `<Link>` to the editor doesn't fire on the affordances.
  Arrows disabled at boundaries.
- Empty state — "No notes on this shelf yet" hero +
  `+ Add notes to shelf` CTA wired to the 11.3b
  `<AddNotesToShelfModal>` (which the detail view reuses
  verbatim — same picker, same pre-fetched eligibles).

**`<RemoveFromShelfConfirm>` extracted.** Pulled out of
`all-shelves-carousel.tsx` into its own file
`lib/library/remove-from-shelf-confirm.tsx` so both surfaces
share the dialog. The note prop is now a `{ note_id; title }`
subset so the same component works whether the caller has a
`LibraryShelfCardNote` (carousel) or a `LibraryShelfDetailNote`
(detail). Pure refactor — no behaviour change on the carousel
side.

**CSS.** New SLICE 11.4 block in `styles/library.css` covers:
shelf-detail title row (dot + badge), sub-line tagline +
description paragraph, the ordered `<ol>` with shelf-accent
left-edge bar, the floating tool group with hover + focus-within
reveal, and the folder + shelf meta-chip variants.

### Files

**New (2):** `lib/library/shelf-detail.tsx`,
`lib/library/remove-from-shelf-confirm.tsx`.

**Modified (8):** `lib/library/types.ts` (LibraryShelfDetail +
LibraryShelfDetailNote), `lib/library/queries.ts` (getShelfDetail),
`lib/library/actions.ts` (reorderShelfMemberAction),
`lib/library/shelf-rows.tsx` (per-shelf href + is-active),
`lib/library/home-shell.tsx` (shelfDetail prop + branched main
pane), `lib/library/all-shelves-carousel.tsx` (import shared
confirm), `app/(app)/tutor/library/page.tsx` (3-way branch +
detail-scope eligibles), `styles/library.css` (SLICE 11.4 block).

**Doc updates:** `BUILD_LIST.md` "Last shipped" + ⏭ pointer →
Slice 11.5 (Tiptap editor scaffold); `docs/product-plan/tutor-library.md`
build-order flag ⬜ → ✅.

### What's next

`BUILD_LIST.md` ⏭ pointer is now **Slice 11.5 — Tiptap editor
scaffold** (with the markdown-textarea fall-back gate per
[the library editor memory](C:\Users\confi\.claude\projects\C--Users-confi-qacademy-mynclex\memory\project_library_editor_choice.md)).
Or rotate per the alternate-features rule — **Payments 5.3** is
the live alternate (5.1 + 5.2 shipped earlier in the period;
5.3+ is the polymorphic-row checkout + the bank opt-in surfaces).

---

## Session — 2026-05-26 (Deploy regression revert + Library Slice 11.3a)

Second session of the day on the new `claude/priceless-colden-554978`
worktree. Two coherent threads: a same-day regression revert that
restored the broken Cloudflare dev deploy, then the next library
slice end-to-end.

### Thread 1 — Revert proxy.ts → middleware.ts (commit `17e62ca`)

The morning's session 11.2b deploy of `d4d08ae` failed; investigation
showed the failure was actually introduced by `2c66d46` ("clear
Next.js 16 startup warnings (middleware → proxy)"). The first deploy
of the proxy-renamed code happened on 11.2b's commit because earlier
commits in the same push (b73215d → 2c66d46 → a2e7e7b → d4d08ae)
were batched — GitHub Actions only runs CI on the push head.

Root cause. Next.js 16's "proxy" model is **Node.js-runtime only**;
route segment config (including `export const runtime = 'edge'`) is
forbidden on proxies. The current `@opennextjs/cloudflare` (1.19.x,
latest 1.19.11) refuses Node middleware with:

    ERROR Node.js middleware is not currently supported.
    Consider switching to Edge Middleware.

Tracking upstream: `cloudflare/workers-sdk#13755` — unresolved.
Next.js 16.2 shipped the stable Build Adapters API specifically to
unblock OpenNext's proxy.ts support, but OpenNext hasn't picked it
up yet.

The fix is a clean revert of the rename half of `2c66d46` (kept the
Turbopack-root pin from the same commit — unrelated, working):
- `git mv proxy.ts middleware.ts` (history preserved)
- exported function back: `proxy()` → `middleware()`
- 3 doc refs back: `CLAUDE.md` rule #6, `lib/access/README.md`,
  `lib/supabase/server.ts` comment
- new `CLAUDE.md` "Known Workarounds" entry naming the upstream
  issue + the condition for trying the rename again (OpenNext ships
  proxy.ts support via the 16.2 Build Adapters API), so a future
  session doesn't re-attempt the rename and re-break the deploy

Cost: the deprecation warning prints at every `npm run dev` startup
again — which is the original cosmetic annoyance the rename was
meant to clear. Local OpenNext build verified clean past the
middleware step before pushing. After merging to `main`, the dev
Worker deploy went green (2m15s) for the first time since 11.2b
landed.

### Thread 2 — Slice 11.3a: shelf entity + sidebar lens (commit pending)

Sub-sliced 11.3 into a + b up front via `AskUserQuestion` (four
scope locks: split / tagline-column / CD-handoff / add-to-shelf
scope). 11.3a is the shelf entity (CRUD + sidebar); 11.3b is the
Spotify-style All Shelves carousel + add-to-shelf flow.

The build is mechanical — closely parallels 11.2a's folder pattern
because the schema shapes mirror (entity + display projection +
form-values + create/edit/delete actions + sidebar rows + modal).
What's shelf-specific:

- **`tagline` column**, separate from `description`. Sam's call
  ("we need both, different purposes"). Migration
  `20260617120000_slice_11_3a_shelf_tagline.sql` adds
  `tagline TEXT NULL`; description retains its role for the shelf
  detail page (11.4). Both are optional, the UI degrades on null.
- **8-swatch colour palette** lifted verbatim from CD's
  `SHELF_PALETTE` in `tutor-library/note-editor.jsx`: Teal / Amber
  / Blue / Purple / Red / Green / Plum / Slate. App-layer enum
  (DB column is free `TEXT NOT NULL`). Smart default in create
  mode = first palette entry not already worn by an existing
  shelf, falls back to entry 0 above 8.
- **Unified create/edit modal** via discriminated `{ mode }` union
  — same shape as `<CohortFormModal>` from 9.2b. Title +
  tagline + description + colour picker. Live preview pill below
  the swatches showing how the shelf will read as a chip with the
  picked colour. Tagline placeholder copy lifted from CD ("Every
  cohort gets these in Week 1.").
- **Sidebar kebab pattern.** Per-shelf row gains a hover-revealed
  `⋮` button that opens a popover menu with Edit / Delete (no
  hover hide while open, click-outside + Escape close). The row
  itself stays a `<Link>` to `?shelf=all` per the scope decision —
  per-shelf detail view ships in 11.4.
- **Delete catches FK 23503** (RESTRICT on
  `_note_attachments.shelf_id`) and surfaces "Can't delete — this
  shelf is attached to a programme. Detach it first." The detach
  UI itself lands with 11.12.
- **`page.tsx` parallel fetch** — folders + shelves via
  `Promise.all`. When `?shelf=…` is set it takes precedence over
  `?folder=…` in the main pane; the per-folder notes fetch skips
  to avoid wasted work. Sidebar renders both lenses fully
  regardless.
- **Main-pane placeholder** for `?shelf=…`: `ShelvesComingSoon`
  with the 📚 glyph + "All shelves" or "No shelves yet" depending
  on the count. Hint copy explains the carousel ships in 11.3b
  and points at + New shelf as the immediate action.

Process notes:
- Hit one mid-build runtime error while Sam was testing live: the
  `LibraryHomeShell` shelves prop was undefined for one fast-refresh
  cycle while `page.tsx` and `home-shell.tsx` edits propagated at
  different speeds. Recovered on its own; documented here as a
  Turbopack-fast-refresh artefact (not real bug).
- `tsc --noEmit` clean against my new code; the 4 remaining errors
  are the same pre-existing `vitest/config` not-installed-locally
  issue (lockfile + CI both fine; local `node_modules` stale).
...
Files modified (6): `lib/library/types.ts` (SHELF_PALETTE +
LibraryShelf / WithCount / FormValues), `lib/library/queries.ts`
(getShelvesForTutor), `lib/library/actions.ts` (create + edit +
delete actions + validateShelf), `lib/library/home-shell.tsx`
(Shelves lens wired + + New shelf toolbar button + NewShelfModal
+ ShelvesComingSoon main pane), `app/(app)/tutor/library/page.tsx`
(parallel fetch + `?shelf=` URL param + scope precedence),
`styles/library.css` (SLICE 11.3a block).

### Thread 3 — Slice 11.3b: All Shelves carousel + add-to-shelf (commit pending)

Closes the 11.3 sub-arc. Two AskUserQuestion scope locks up front:
draft-or-published eligibility (recommendation accepted: BOTH
eligible — shelves don't gate visibility) and per-card remove
affordance (recommendation accepted: hover-revealed ✕ on each
card).

The architecture mirrors 11.2's read/write split, just at the M:N
membership level:

- **`getShelvesWithNotes()`** — one round trip via PostgREST
  embed: `_shelves → _shelf_memberships → _notes`. Membership
  rows arrive unordered; flattened + sorted by
  membership.position in JS (arrays are small). Returns rich
  `LibraryShelfWithNotes[]` for the carousel.
- **`getEligibleNotesForShelf(shelfId)`** — eligibility = every
  owned note NOT already on this shelf, both DRAFT and
  PUBLISHED. Joins folder name + counts memberships on OTHER
  shelves for the "also on N shelves" badge. Sorted by
  updated_at desc.
- **`attachNotesToShelfAction(shelfId, noteIds)`** — bulk INSERT
  of memberships with `position = current_count + i`. Catches
  23505 (race-attached duplicate) with friendly copy.
- **`removeNoteFromShelfAction(shelfId, noteId)`** — single
  DELETE on the composite-PK `_shelf_memberships`.

UI shipped:

- **`<AllShelvesCarousel>`** replaces the 11.3a `<ShelvesComingSoon>`
  placeholder. Empty-state hero retained as `<ShelvesEmptyHero>`
  when the tutor has 0 shelves (carousel doesn't render in that
  branch).
- **CD-faithful carousel cards** — 244px grid-auto-columns +
  scroll-snap-x; 3px shelf-coloured left-edge bar (`::before` with
  `--shelf-accent` CSS var); 2-line clamp on title + desc; hover
  raises by 1px with a subtle shadow. Trailing dashed
  `.lib-shelf-card-add` tile reuses the same height + scroll-snap.
- **`<AddNotesToShelfModal>`** — multi-select picker with a
  toolbar (search + folder dropdown), summary row (N eligible · M
  selected + Select all / Clear text buttons), scrollable list
  capped at 340px. Per-row chrome: 22px checkbox + main text
  block + Pub/Draft pill on the right. Shelf-colour treatment
  on the modal title (small square dot + the shelf name in the
  shelf's colour).
- **Hover-revealed ✕** on each carousel card; opens a tiny
  `RemoveFromShelfConfirm` (yes/no, prog-btn-danger) with
  reassurance copy ("the note stays put, only the membership
  goes"). Click-stop on the ✕ so the underlying card-link
  doesn't fire.

`page.tsx` reshape:

- When `?shelf=…` is set, fetch the rich
  `getShelvesWithNotes()` projection AND derive the lean sidebar
  shape from it (avoid a second round trip). Otherwise fetch
  only the lean `getShelvesForTutor()`. Saves one query in the
  carousel branch.
- Pre-fetch `getEligibleNotesForShelf(s.shelf_id)` for every
  shelf in parallel via `Promise.all` when the shelf scope is
  active, so opening the picker is instant (no round trip on
  the click).

Notes on the mid-build hiccups:
- One transient "Rendered more hooks than during the previous
  render" hot-reload warning while inserting `useState newShelfOpen`
  between two existing useStates in `home-shell.tsx`. Self-cleared
  on full reload — fast-refresh artefact, not a real bug.
- Dev server hit its memory threshold and auto-restarted during
  testing (74s page render shortly before — Turbopack stumbling
  on the size of the freshly-added components). Restart cleared
  the page-render times back to 5s range.
...
LibraryShelfCardNote / LibraryShelfWithNotes / LibraryEligibleNote),
`lib/library/queries.ts` (getShelvesWithNotes +
getEligibleNotesForShelf), `lib/library/actions.ts` (attach +
remove membership), `lib/library/home-shell.tsx` (mount carousel
+ ShelvesEmptyHero), `app/(app)/tutor/library/page.tsx` (shelf-
scope-aware fetch shape + parallel eligibles), `styles/library.css`
